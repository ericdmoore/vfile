  ...
  for (const matcher of ignores) {
    ...
    if (matcher.startsWith("!")) {
      ...
      shouldIgnore = shouldIgnore ||
        minimatch(relativeFilePath, matcher, MINIMATCH_OPTIONS);
    }
  }
...
function pathMatches(filePath, basePath, config) {
  ...
  if (filePathMatchesPattern && config.ignores) {
    filePathMatchesPattern = !shouldIgnoreFilePath(
      config.ignores,
      filePath,
      relativeFilePath,
    );
...
class ConfigArray extends Array {
  ...
  normalizeSync(context = {}) {
    if (!this.isNormalized()) {
      ...
      this.length = 0;
      this.push(
        ...normalizedConfigs.map(this[ConfigArraySymbol.preprocessConfig]),
      );
      this[ConfigArraySymbol.isNormalized] = true;
  ...
  getConfig(filePath) {
    ...
    finalConfig = matchingConfigs.reduce(
      (result, config) => {
