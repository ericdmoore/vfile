...
var sortedFlags = availableFlags.slice().sort(function (a, b) {
...
module.exports = function runTests(flags, t) {
  ...
  t.equal(
    flags(new RegExp("a", "gmi")),
    "gim",
    'flags(new RegExp("a", "gmi")) !== "gim"',
  );
  ...
  forEach(availableFlags, function (flag) {
    ...
    t.test(property + " flag", function (st) {
      st.equal(
        flags(getRegexLiteral("/a/" + flag)),
        flag,
        "flags(/a/" + flag + ") !== " + inspect(flag),
      );
      st.equal(
        flags(new RegExp("a", flag)),
        flag,
        'flags(new RegExp("a", ' + inspect(flag) + ")) !== " + inspect(flag),
      );
      st.end();
    });
  });
  ...
  t.test("sorting", function (st) {
    ...
    if (has(RegExp.prototype, "dotAll")) {
      st.equal(
        flags(getRegexLiteral("/a/sgmi")),
        "gims",
        'flags(/a/sgmi) !== "gims"',
      );
    }

    var randomFlags = availableFlags.slice().sort(function () {
  ...
  t.test("generic flags", function (st) {
    ...
    st.equal(
      flags(obj),
      sortedFlags,
      "an object with every available flag: " + sortedFlags,
    );
  ...
  t.test("throws properly", function (st) {
    ...
    var throwsOnNonObject = function (nonObject) {
      st["throws"](
        flags.bind(null, nonObject),
        TypeError,
        inspect(nonObject) + " is not an Object",
      );
    };
