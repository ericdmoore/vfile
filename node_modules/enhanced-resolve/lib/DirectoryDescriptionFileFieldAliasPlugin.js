...
function findDescriptionFileField(
  resolver,
  directory,
  filename,
  field,
  ...
  (function findDescriptionFile() {
    ...
    resolver.fileSystem.readFile(descriptionFilePath, function (err, content) {
      ...
      try {
        ...
        if (callback.log) {
          callback.log(
            descriptionFilePath + " (directory description file): " + e,
          );
        } else {
          e.message = descriptionFilePath + " (directory description file): " +
            e;
        }
        return callback(e);
      }
...
DirectoryDescriptionFileFieldAliasPlugin.prototype.apply = function (resolver) {
  ...
  resolver.plugin("module", function (request, callback) {
    ...
    findDescriptionFileField(
      this,
      directory,
      filename,
      field,
      function (err, fieldData, directory) {
        ...
        var newCallback = createInnerCallback(
          callback,
          callback,
          "aliased from directory description file " +
            this.join(directory, filename) + " with mapping " +
            JSON.stringify(moduleName),
        );
        if (newRequest.module) {
          return this.doResolve("module", obj, newCallback);
        }
        if (newRequest.directory) {
          return this.doResolve("directory", obj, newCallback);
        }
  ...
  resolver.plugin("result", function (request, callback) {
    ...
    findDescriptionFileField(
      ...
      function (err, fieldData, directory) {
        if (err) return callback(err);
        if (!fieldData) return callback();
        var relative = requestPath.substr(directory.length + 1).replace(
          /\\/g,
          "/",
        );
