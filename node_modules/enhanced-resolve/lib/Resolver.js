...
Resolver.prototype.doResolve = function doResolve(
  types,
  request,
  callback,
  noError,
  ...
  var currentRequestString = request.request
    ? request.request + " in " + request.path
    : request.path;
  if (types.length == 1 && !noError) {
    // If only one type, we can pass the error.
    return this.applyPluginsParallelBailResult(
      types[0],
      request,
      createInnerCallback(function innerCallback(err, result) {
        ...
        var error = new Error(
          "Cannot resolve " + types[0] + " '" + request.request + "' in " +
            request.path,
        );
  ...
  this.forEachBail(
    ...
    function (result) {
      if (callback.log) {
        callback.log(
          "resolve '" + types.join("' or '") + "' " + currentRequestString,
        );
        for (var i = 0; i < log.length; i++) {
          callback.log("  " + log[i]);
        }
      ...
      if (result) return callback(null, result);
      var error = new Error(
        "Cannot resolve '" + types.join("' or '") + "' " + currentRequestString,
      );
      error.details = logAsString();
      error.missing = localMissing;
      return callback(error);
