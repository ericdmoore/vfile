...
module.exports = function GetSubstitution(
  ...
  if (!IsArray(captures) || !every(captures, isStringOrHole)) {
    throw new $TypeError(
      "Assertion failed: `captures` must be a possibly-empty List of Strings, got " +
        inspect(captures),
    );
  }
  ...
  for (var i = 0; i < replacement.length; i += 1) {
    ...
    if (current === "$" && !isLast) {
      ...
      if (next === "$") {
        ...
        if (
          isDigit(next) && next !== "0" && (nextIsLast || !isDigit(nextNext))
        ) {
          // $1 through $9, and not followed by a digit
          var n = $parseInt(next, 10);
          // if (n > m, impl-defined)
          result += n <= m && Type(captures[n - 1]) === "Undefined"
            ? ""
            : captures[n - 1];
          i += 1;
          ...
          result += nn <= m && Type(captures[nnI]) === "Undefined"
            ? ""
            : captures[nnI];
          i += 2;
        } else if (next === "<") {
