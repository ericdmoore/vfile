...
var functionsHaveConfigurableLengths = !!(
  ...
  Object.getOwnPropertyDescriptor(bind.call(function () {}), "length")
    .configurable
);
...
test("callBind", function (t) {
  ...
  t.test("callBind.apply", function (st) {
    ...
    st.deepEqual(
      aBound(sentinel, [1, 2], 4),
      [sentinel, 1, 2],
      "apply-bound func with right args",
    );

    var aBoundArg = callBind.apply(func);
    st.deepEqual(
      aBoundArg(sentinel, [1, 2, 3], 4),
      [sentinel, 1, 2],
      "apply-bound func with too many args",
    );
    st.deepEqual(
      aBoundArg(sentinel, [1, 2], 4),
      [sentinel, 1, 2],
      "apply-bound func with right args",
    );
    st.deepEqual(
      aBoundArg(sentinel, [1], 4),
      [sentinel, 1, undefined],
      "apply-bound func with too few args",
    );
    ...
    st.deepEqual(
      aBoundR([1, 2, 3], 4),
      [sentinel, 1, 2],
      "apply-bound func with receiver and too many args",
    );
    st.deepEqual(
      aBoundR([1, 2], 4),
      [sentinel, 1, 2],
      "apply-bound func with receiver and right args",
    );
    st.deepEqual(
      aBoundR([1], 4),
      [sentinel, 1, undefined],
      "apply-bound func with receiver and too few args",
    );
