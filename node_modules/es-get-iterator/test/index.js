...
var getIterator = process.env.TEST_VARIANT === "node"
  ? require("../node")
  : require("../");
...
Test.prototype.iterate = function (value, expected, message) {
  ...
  this.test(message, function (t) {
    ...
    if (typeof iterator.next !== "function") {
      t.fail(
        "iterator does not have a next function, got " + inspect(iterator),
      );
      return t.end();
    }
    var result;
    while ((result = iterator.next()) && !result.done) {
      t.deepEqual(
        result.value,
        expected[i],
        "index " + i + ": expected " + inspect(expected[i]) + ", got " +
    ...
    t.equal(
      i,
      expected.length,
      "expected " + expected.length + " values, got " + i + " values",
    );
...
Test.prototype.noIterate = function (value) {
  this.equal(
    getIterator(value),
    undefined,
    inspect(value) + " is not iterable",
  );
};

Test.prototype.fakeIterator = function (value) {
  this.test(
    inspect(value) + " with a fake iterator",
    { skip: !hasSymbols },
    function (t) {
      ...
      t.iterate(
        ...
        fakeValues,
        inspect(o) +
          " with an overwritten iterator method, yields those values instead",
      );
      t.end();
    },
  );
...
var runTests = function runTests(t) {
  t.test("strings", function (st) {
    ...
    st.iterate(
      Object("foo"),
      ["f", "o", "o"],
      inspect(Object("foo")) + " yields three chars",
    );
    st.iterate("a💩z", ["a", "💩", "z"], '"a💩z" yields three code points');
    st.iterate(
      Object("a💩z"),
      ["a", "💩", "z"],
      inspect(Object("a💩z")) + " yields three code points",
    );
    st.iterate(
      "\ud83dX",
      ["\ud83d", "X"],
      inspect("\ud83dX") +
        ' (lone surrogate followed by "not a lone surrogate ending") yields one code point',
    );
  ...
  t.test("arguments", function (st) {
    ...
    st.iterate(
      getSloppyArguments(),
      [],
      "empty sloppy arguments object yields nothing",
    );
    st.iterate(
      getArguments(1, 2, 3),
      [1, 2, 3],
      "arguments object yields all args",
    );
    st.iterate(
      getSloppyArguments(1, 2, 3),
      [1, 2, 3],
      "sloppy arguments object yields all args",
    );
  ...
  t.test("non-iterables", function (st) {
    ...
    if (hasSymbols && NaN[Symbol.iterator]) {
      st.comment(
        "# SKIP core-js v2 makes numbers iterable, in violation of the spec",
      );
    }
...
test(
  (process.env.TEST_VARIANT || "standard") + ": getIterator tests",
  function (t) {
    runTests(t);
