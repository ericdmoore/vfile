...
exports
  .recursivePatternCapture = recursivePatternCapture;
var _fs = require("fs");
    ...
    value: function () {
      function has(name) {
        ...
        if (name !== "default") {
          ...
          try {
            for (
              var _iterator = this.dependencies[Symbol.iterator](), _step;
    ...
    value: function () {
      function hasDeep(name) {
        ...
        if (name !== "default") {
          ...
          try {
            for (
              var _iterator2 = this.dependencies[Symbol.iterator](), _step2;
              !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done);
              _iteratorNormalCompletion2 = true
            ) {
              var dep = _step2.value;
              var innerMap = dep();
              if (innerMap == null) return { found: true, path: [this] }; // todo: report as unresolved?
    ...
    value: function () {
      function get(name) {
        ...
        if (name !== "default") {
          ...
          try {
            for (
              var _iterator3 = this.dependencies[Symbol.iterator](), _step3;
          ...
          message: "Parse errors in imported module '" +
            ...
            ("" + String(
              this.errors.map(function (e) {
                return String(e.message) + " (" + String(e.lineNumber) + ":" +
                  String(e.column) + ")";
              }).join(", "),
            )),
        });
...
function captureDoc(source, docStyleParsers) {
  ...
  nodes.some(function (n) {
    try {
      ...
      if ("leadingComments" in n) leadingComments = n.leadingComments;
...
function captureJsDoc(comments) {
  ...
  comments.forEach(function (comment) { // skip non-block comments
    ...
    } catch (err) { /* don't care, for now? maybe add to `errors?` */ }
  });
  return doc;
} /**
...
function captureTomDoc(comments) { // collect lines up to first paragraph break
  ...
  var statusMatch = lines.join(" ").match(
...
var supportedImportTypes = new Set([
...
ExportMap["for"] = function (context) {
  ...
  if (exportMap != null) { // date equality check
  ...
  var content = _fs2["default"].readFileSync(path, { encoding: "utf8" }); // check for and cache unambiguous modules
...
ExportMap.parse = function (path, content, context) {
  ...
  var docstyle = context.settings && context.settings["import/docstyle"] ||
    ["jsdoc"];
  var docStyleParsers = {};
  ...
  function processSpecifier(s, n, m) {
    ...
    switch (s.type) {
      ...
      case "ExportAllDeclaration":
        ...
        return;
      case "ExportSpecifier": // else falls through
        if (!n.source) {
  ...
  function captureDependency(_ref, isOnlyImportingTypes) {
    ...
    var importedSpecifiers = arguments.length > 2 && arguments[2] !== undefined
      ? arguments[2]
      : new Set();
    if (source == null) return null;
    var p = remotePath(source.value);
    if (p == null) return null;
    var declarationMetadata = { // capturing actual node reference holds full AST in memory!
  ...
  function readTsConfig() {
    var tsConfigInfo = (0, _tsconfigLoader.tsConfigLoader)({
      cwd: context.parserOptions && context.parserOptions.tsconfigRootDir ||
        process.cwd(),
      getEnv: function () {
        function getEnv(key) {
    ...
    try {
      if (tsConfigInfo.tsConfigPath !== undefined) { // Projects not using TypeScript won't have `typescript` installed.
        if (!ts) ts = require("typescript");
        var configFile = ts.readConfigFile(
        ...
        return ts.parseJsonConfigFileContent(
          configFile.config,
          ts.sys,
          (0, _path.dirname)(tsConfigInfo.tsConfigPath),
        );
      }
  ...
  ast.body.forEach(function (n) {
    ...
    if (n.type === "ExportAllDeclaration") {
      ...
      if (n.exported) processSpecifier(n, n.exported, m);
      return;
    ...
    if (n.type === "ExportNamedDeclaration") { // capture declaration
    ...
    if (isEsModuleInteropTrue) exports.push("TSNamespaceExportDeclaration"); // This doesn't declare anything, but changes what's being exported.
    if ((0, _arrayIncludes2["default"])(exports, n.type)) {
      var exportedName = n.type === "TSNamespaceExportDeclaration"
        ? n.id.name
        : n.expression && n.expression.name ||
          n.expression.id && n.expression.id.name || null;
      var declTypes = [
        "VariableDeclaration",
        "ClassDeclaration",
        "TSDeclareFunction",
      ...
      var exportedDecls = ast.body.filter(function (_ref2) {
        ...
        return (0, _arrayIncludes2["default"])(declTypes, type) &&
          (id && id.name === exportedName ||
            declarations && declarations.find(function (d) {
                return d.id.name === exportedName;
              }));
      });
      if (exportedDecls.length === 0) { // Export is not referencing any local declaration, must be re-exporting
      ...
      exportedDecls.forEach(function (decl) {
        if (decl.type === "TSModuleDeclaration") {
          if (decl.body && decl.body.type === "TSModuleDeclaration") {
            ...
            decl.body.body.forEach(function (moduleBlockNode) { // Export-assignment exports all members in the namespace,
  ...
  if (unambiguouslyESM) m.parseGoal = "Module";
  return m;
}; /**
...
function makeSourceCode(text, ast) {
  if (_eslint.SourceCode.length > 1) { // ESLint 3
