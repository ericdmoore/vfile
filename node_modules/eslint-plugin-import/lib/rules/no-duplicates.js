...
function checkImports(imported, context) {
  ...
  try {
    for (
      var _iterator = imported.entries()[Symbol.iterator](), _step;
      ...
      if (nodes.length > 1) {
        ...
        try {
          for (
            var _iterator2 = rest[Symbol.iterator](), _step2;
...
function getFix(first, rest, sourceCode) {
  ...
  var defaultImportNames = new Set(
    [first].concat(_toConsumableArray(rest)).map(getDefaultImportName).filter(
      Boolean,
    ),
  );
  ...
  var unnecessaryImports = restWithoutComments.filter(function (node) {
    return (
      !hasSpecifiers(node) &&
      !hasNamespace(node) &&
      !specifiers.some(function (specifier) {
        return specifier.importNode === node;
      })
    );
  });

  var shouldAddDefault = getDefaultImportName(first) == null &&
    defaultImportNames.size === 1;
  ...
  return function (fixer) {
    ...
    var firstHasTrailingComma = closeBrace != null &&
    ...
    if (shouldAddDefault && openBrace == null && shouldAddSpecifiers) {
      ...
      fixes.push(
        fixer.insertTextAfter(
          firstToken,
          " " + String(defaultImportName) + ", {" + String(specifiersText) +
            "} from",
        ),
      );
    ...
    try {
      for (
        var _iterator4 = unnecessaryImports[Symbol.iterator](), _step4;
...
function hasCommentInsideNonSpecifiers(node, sourceCode) {
  ...
  var someTokens = openBraceIndex >= 0 && closeBraceIndex >= 0
    ? tokens.slice(1, openBraceIndex + 1).concat(
...
module.exports = {
  ...
  create: function () {
    function create(context) {
      ...
      var resolver = considerQueryStringOption
        ? function (sourcePath) {
