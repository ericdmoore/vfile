  ...
  create: function () {
    function create(context) {
      ...
      function handleImportDefault(node) {
        ...
        var exportMap = _ExportMap2["default"].get(
      ...
      function handleDestructuringAssignment(node) {
        ...
        try {
          for (
            var _iterator = node.id.properties[Symbol.iterator](), _step;
      ...
      function handleProgramExit() {
        allPropertyLookups.forEach(function (lookups, objectName) {
          ...
          try {
            for (
              var _iterator2 = lookups[Symbol.iterator](), _step2;
              ...
              context.report({
                node: node,
                message: "Caution: `" + String(objectName) +
                  "` also has a named export " + ("`" + String(
                    propName,
                  ) + "`. Check if you meant to write ") +
                  ("`import {" + String(
                    propName,
                  ) + "} from '" + String(fileImport.sourcePath) + "'` ") +
                  "instead.",
              });
            }
          } catch (err) {
