...
module.exports = {
  ...
  create: function () {
    function noRestrictedPaths(context) {
      ...
      var currentFilename = context.getPhysicalFilename
      ...
      function isValidExceptionPath(absoluteFromPath, absoluteExceptionPath) {
        var relativeExceptionPath = _path2["default"].relative(
          absoluteFromPath,
          absoluteExceptionPath,
        );

        return (0, _importType2["default"])(relativeExceptionPath, context) !==
          "parent";
      }
      ...
      function reportInvalidExceptionGlob(node) {
        context.report({
          node: node,
          message:
            "Restricted path exceptions must be glob patterns when`from` is a glob pattern",
        });
      }

      var makePathValidator = function () {
        function makePathValidator(zoneFrom) {
          var zoneExcept = arguments.length > 1 && arguments[1] !== undefined
            ? arguments[1]
            : [];
          ...
          if (isGlobPattern) {
            ...
            hasValidExceptions = absoluteExceptionPaths
              .every(function (absoluteExceptionPath) {
                return isValidExceptionPath(
                  absoluteFrom,
                  absoluteExceptionPath,
                );
              });
      ...
      function checkForRestrictedImportPath(importPath, node) {
        ...
        matchingZones.forEach(function (zone, index) {
          ...
          context.report({
            node: node,
            message:
              'Unexpected path "{{importPath}}" imported in restricted zone.' +
              (zone.message ? " " + String(zone.message) : ""),
