...
function findEndOfLineWithComments(sourceCode, node) {
  ...
  for (var i = endOfTokens; i < sourceCode.text.length; i++) {
    ...
    if (
      sourceCode.text[i] !== " " && sourceCode.text[i] !== "\t" &&
      sourceCode.text[i] !== "\r"
    ) {
      break;
    }
...
function findStartOfLineWithComments(sourceCode, node) {
  var tokensToEndOfLine = takeTokensBeforeWhile(
    sourceCode,
    node,
    commentOnSameLineAs(node),
  );
  var startOfTokens = tokensToEndOfLine.length > 0
...
function isPlainImportModule(node) {
  return node.type === "ImportDeclaration" && node.specifiers != null &&
...
function canReorderItems(firstNode, secondNode) {
  ...
  try {
    for (
      var _iterator = nodesBetween[Symbol.iterator](), _step;
...
function fixOutOfOrder(context, firstNode, secondNode, order) {
  ...
  var message = "`" + String(secondNode.displayName) +
    "` import should occur " + String(order) + " import of `" +
    String(firstNode.displayName) + "`";
...
function mutateRanksToAlphabetize(imported, alphabetizeOptions) {
  ...
  var alphabetizedRanks = groupRanks.sort().reduce(function (acc, groupRank) {
    groupedByRanks[groupRank].forEach(function (importedItem) {
      acc[
        String(importedItem.value) + "|" + String(importedItem.node.importKind)
...
function computePathRank(ranks, pathGroups, path, maxPosition) {
  for (var i = 0, l = pathGroups.length; i < l; i++) {
    var _pathGroups$i = pathGroups[i],
      ...
      position = _pathGroups$i$positio === undefined
        ? 1
        : _pathGroups$i$positio;
    if (
      (0, _minimatch2["default"])(
        path,
        pattern,
...
function computeRank(context, ranks, importEntry, excludedImportTypes) {
  ...
  if (
    importEntry.type !== "import" && !importEntry.type.startsWith("import:")
  ) {
    rank += 100;
  }
  ...
  var rankObject = groups.reduce(function (res, group, index) {
    ...
    group.forEach(function (groupItem) {
      ...
      if (res[groupItem] !== undefined) {
        throw new Error(
          "Incorrect configuration of the rule: `" + groupItem +
            "` is duplicated",
        );
      }
      res[groupItem] = index;
...
function makeNewlinesBetweenReport(context, imported, newlinesBetweenImports) {
  ...
  imported.slice(1).forEach(function (currentImport) {
    ...
    if (
      ...
      if (
        currentImport.rank !== previousImport.rank && emptyLinesBetween === 0
      ) {
        context.report({
...
module.exports = {
  ...
  create: function () {
    function importOrderRule(context) {
      ...
      var pathGroupsExcludedImportTypes = new Set(
      ...
      try {
        var _convertPathGroupsFor = convertPathGroupsForRanks(
            options.pathGroups || [],
          ),
          pathGroups = _convertPathGroupsFor.pathGroups,
          maxPosition = _convertPathGroupsFor.maxPosition;
        var _convertGroupsToRanks = convertGroupsToRanks(
        ...
        TSImportEqualsDeclaration: function () {
          function handleImports(node) {
            ...
            if (node.moduleReference.type === "TSExternalModuleReference") {
              ...
              value = "";
              displayName = context.getSourceCode().getText(
