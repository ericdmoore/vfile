...
function findScope(context, identifier) {
  ...
  return scopeManager &&
    scopeManager.scopes.slice().reverse().find(function (scope) {
  ...
  create: function () {
    function create(context) {
      ...
      function report(node) {
        ...
        var hasCJSExportReference = hasKeywords &&
          (!objectScope || objectScope.type === "module");
        var isException = !!options.exceptions &&
          options.exceptions.some(function (glob) {
            return (0, _minimatch2["default"])(fileName, glob);
          });

        if (
          isIdentifier && hasCJSExportReference && !isEntryPoint && !isException
        ) {
          importDeclarations.forEach(function (importDeclaration) {
            context.report({
              node: importDeclaration,
              message:
                "Cannot use import declarations in modules that export using " +
