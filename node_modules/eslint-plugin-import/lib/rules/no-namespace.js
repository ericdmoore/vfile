      ...
      return {
        ImportNamespaceSpecifier: function () {
          function ImportNamespaceSpecifier(node) {
            if (
              ignoreGlobs && ignoreGlobs.find(function (glob) {
                return (0, _minimatch2["default"])(
            ...
            var namespaceIdentifiers = namespaceReferences.map(
              function (reference) {
                return reference.identifier;
              },
            );
            var canFix = namespaceIdentifiers.length > 0 &&
              ...
              fix: canFix && function (fixer) {
                ...
                namespaceIdentifiers.forEach(function (identifier) {
                  ...
                  if (parent && parent.type === "MemberExpression") {
                    ...
                    var localConflicts = getVariableNamesInScope(
                      scopeManager,
                      parent,
                    );
                ...
                var namedImportSpecifiers = importNames.map(
                  function (importName) {
                    return (
                ...
                fixes.push(
                  fixer.replaceText(
                    node,
                    "{ " + String(namedImportSpecifiers.join(", ")) + " }",
                  ),
                );
...
function getVariableNamesInScope(scopeManager, node) {
  ...
  return new Set([].concat(
    ...
    _toConsumableArray(
      scope.upper.variables.map(function (variable) {
        return variable.name;
      }),
    ),
  ));
...
function generateLocalNames(names, nameConflicts, namespaceName) {
  ...
  names.forEach(function (name) {
    ...
    if (!nameConflicts[name].has(name)) {
      ...
      !nameConflicts[name].has(String(namespaceName) + "_" + String(name))
    ) {
      localName = String(namespaceName) + "_" + String(name);
    } else {
      for (var i = 1; i < Infinity; i++) {
        if (
          !nameConflicts[name].has(
            String(namespaceName) + "_" + String(name) + "_" + String(i),
          )
        ) {
          localName = String(namespaceName) + "_" + String(name) + "_" +
            String(i);
          break;
        }
      }
