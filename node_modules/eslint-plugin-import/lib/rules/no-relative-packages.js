...
function checkImportForRelativePackage(context, importPath, node) {
  ...
  if (
    potentialViolationTypes.indexOf(
      (0, _importType2["default"])(importPath, context),
    ) === -1
  ) {
    return;
  }
  ...
  if (
    ...
    var properImport = _path2["default"].join(
      ...
      importBaseName === _path2["default"].basename(importRoot)
        ? ""
        : importBaseName,
    );
    ...
    context.report({
      node: node,
      message: "Relative import from another package is not allowed. Use `" +
        String(properImport) + "` instead of `" + String(importPath) + "`",
    });
  }
}
