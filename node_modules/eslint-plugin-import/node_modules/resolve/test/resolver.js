...
test("cup", function (t) {
  ...
  resolve("./cup", {
    ...
    t.equal(
      err.message,
      "Cannot find module './cup' from '" + path.join(dir, "cupboard.js") + "'",
    );
  });
});
...
test(
  '#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name',
  function (t) {
    t.plan(2);
...
test(
  "async: #121 - treating an existing file as a dir when no basedir",
  function (t) {
...
test("malformed package.json", function (t) {
  ...
  resolve("./index.js", { basedir: basedir }, function (err, res, pkg) {
    ...
    t.equal(
      pkg,
      undefined,
      "malformed package.json gives an undefined `pkg` argument",
    );
  });
  ...
  resolve(
    ...
    function (err, res, pkg) {
      ...
      t.equal(
        pkg,
        undefined,
        "with packageFilter: malformed package.json gives an undefined `pkg` argument",
      );
    },
  );
  ...
  resolve(
    "./index.js",
    {
      ...
      readPackage: function (readFile, pkgfile, cb) {
        t.equal(
          ...
          "readPackageSync: `pkgfile` is package.json path",
        );
        readFile(pkgfile, function (err, result) {
          try {
            ...
            t.ok(
              ...
              "readPackage: malformed package.json parses as a syntax error",
            );
            cb(null);
          }
        });
    ...
    function (err, res, pkg) {
      ...
      t.equal(
        res,
        expected,
        "with readPackage: malformed package.json is silently ignored",
      );
      t.equal(
        pkg,
        undefined,
        "with readPackage: malformed package.json gives an undefined `pkg` argument",
      );
    },
  );
});
