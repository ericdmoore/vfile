...
test("`./sync` entry point", function (t) {
  t.equal(
    resolve.sync,
    sync,
    "`./sync` entry point is the same as `.sync` on `main`",
  );
  t.end();
});
...
test(
  "sync: #121 - treating an existing file as a dir when no basedir",
  function (t) {
      ...
      readPackageSync: function (readFileSync, pkgfile) {
        ...
        try {
          ...
          t.ok(
            ...
            "readPackageSync: malformed package.json parses as a syntax error",
          );
        }
      },
    },
