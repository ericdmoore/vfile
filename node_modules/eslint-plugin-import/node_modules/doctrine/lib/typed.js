...
(function () {
  ...
  function isTypeName(ch) {
    return "><(){}[],:*|?!=".indexOf(String.fromCharCode(ch)) === -1 &&
      !esutils.code.isWhiteSpace(ch) && !esutils.code.isLineTerminator(ch);
  }
  ...
  function next() {
    ...
    while (
      index < length && esutils.code.isWhiteSpace(source.charCodeAt(index))
    ) {
      advance();
    }
    if (index >= length) {
  ...
  function parseTypeExpression() {
    ...
    if (token === Token.QUESTION) {
      ...
      if (
        token === Token.COMMA || token === Token.EQUAL ||
        token === Token.RBRACE ||
    ...
    if (token === Token.LBRACK) {
      ...
      expect(
        Token.RBRACK,
        "expected an array-style type declaration (" + value + "[])",
      );
      return maybeAddRange({
        type: Syntax.TypeApplication,
