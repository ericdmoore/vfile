...
resolveFrom$2.exports.silent = (fromDir, moduleId) =>
  resolveFrom$1(fromDir, moduleId, true);
...
const parseError = (e, txt, context) => {
  ...
  const msg = badToken
    ? e.message.replace(
      /^Unexpected token ./,
      `Unexpected token ${JSON.stringify(badToken[1])} (${
        hexify(badToken[1])
      })`,
    )
    : e.message;
  ...
  if (errIdx !== null && errIdx !== undefined) {
    ...
    const slice = (start === 0 ? "" : "...") + txt.slice(start, end) +
      (end === txt.length ? "" : "...");
...
convert$1.rgb.ansi256 = function (args) {
  ...
  var ansi = 16 + 36 * Math.round(r / 255 * 5) + 6 * Math.round(g / 255 * 5) +
    Math.round(b / 255 * 5);
  return ansi;
};
...
function supportsColor(stream) {
  ...
  if ("CI" in env) {
    if (
      ["TRAVIS", "CIRCLECI", "APPVEYOR", "GITLAB_CI"].some((sign) =>
        sign in env
...
(function (module) {
  ...
  const isSimpleWindowsTerm = process.platform === "win32" &&
...
function codeFrameColumns$1(rawLines, loc, opts = {}) {
  ...
  const highlightedLines = highlighted
    ...
    : rawLines;
  let frame = highlightedLines.split(NEWLINE, end).slice(start, end).map(
    (line, index) => {
...
class Document$3 extends PlainValue$5.Node {
  ...
  toString() {
    ...
    if (contents.length > 0) {
      if (
        directives.length > 0 || contents[0].type === PlainValue$5.Type.COMMENT
      ) {
        str += "---\n";
      }
      str += contents.join("");
      ...
      if (!this.blockIndent) {
        ...
        if (!this.error) {
          const src = explicit
            ...
            : "first line";
          const msg =
            `Block scalars must not be less indented than their ${src}`;
          this.error = new PlainValue$5.YAMLSemanticError(this, msg);
        }
      }
...
class ParseContext {
  ...
  parseProps(offset) {
    ...
    while (
      ch === PlainValue$5.Char.ANCHOR || ch === PlainValue$5.Char.COMMENT ||
      ch === PlainValue$5.Char.TAG || ch === "\n"
    ) {
      if (ch === "\n") {
        let inEnd = offset;
        let lineStart;
        ...
        if (
          ch === PlainValue$5.Char.TAG && src[end] === "," &&
...
class Collection extends Node {
  ...
  toString(
    ...
    const inFlow = this.type === PlainValue$4.Type.FLOW_MAP ||
      this.type === PlainValue$4.Type.FLOW_SEQ || ctx.inFlow;
...
class Pair extends Node {
  ...
  toString(ctx, onComment, onChompKeep) {
    ...
    if (
      ...
      value instanceof YAMLSeq && value.type !== PlainValue$4.Type.FLOW_SEQ &&
...
function blockString(
  ...
  const literal = type === PlainValue$4.Type.BLOCK_FOLDED
    ? false
...
function plainString(item, ctx, onComment, onChompKeep) {
  ...
  if (
    !implicitKey && !inFlow && type !== PlainValue$4.Type.PLAIN &&
    value.indexOf("\n") !== -1
  ) {
...
const isCollectionItem = (node) => {
  ...
  return type === PlainValue$4.Type.MAP_KEY ||
    type === PlainValue$4.Type.MAP_VALUE || type === PlainValue$4.Type.SEQ_ITEM;
};
...
var ordinaryToPrimitive$1 = function (input, pref) {
  var fn, val;
  if (
    pref === "string" && isCallable$5(fn = input.toString) &&
    !isObject$4(val = call$2(fn, input))
  ) {
    return val;
  }
  if (
    isCallable$5(fn = input.valueOf) && !isObject$4(val = call$2(fn, input))
  ) {
    return val;
  }
  if (
...
objectDefineProperty.f = DESCRIPTORS$2
  ...
  : function defineProperty(O, P, Attributes) {
    ...
    if ("get" in Attributes || "set" in Attributes) {throw TypeError$2(
...
var nativeWeakMap = isCallable$3(WeakMap$1) &&
...
(redefine$1.exports = function (O, key, value, options) {
  ...
  return isCallable$2(this) && getInternalState(this).source ||
    inspectSource(this);
});
...
function parsePairs(doc, cst) {
  ...
  for (let i = 0; i < seq.items.length; ++i) {
    ...
    seq.items[i] = item instanceof resolveSeq$2.Pair
      ? item
      : new resolveSeq$2.Pair(item);
  }
...
class YAMLSet extends resolveSeq$2.YAMLMap {
  ...
  add(key) {
    const pair = key instanceof resolveSeq$2.Pair
      ? key
      : new resolveSeq$2.Pair(key);
  ...
  get(key, keepPair) {
    ...
    return !keepPair && pair instanceof resolveSeq$2.Pair
      ? pair.key instanceof resolveSeq$2.Scalar ? pair.key.value : pair.key
      : pair;
  }
...
function warnFileDeprecation(filename) {
  if (shouldWarn(true)) {
    const path = filename.replace(/.*yaml[/\\]/i, "").replace(/\.js$/, "")
      .replace(/\\/g, "/");
    warn(
...
function getSchemaTags(schemas, knownTags, customTags, schemaId) {
  ...
  for (let i = 0; i < tags.length; ++i) {
    ...
    if (typeof tag === "string") {
      ...
      if (!tagObj) {
        const keys = Object.keys(knownTags).map((key) => JSON.stringify(key))
          .join(", ");
...
function getTagObject(tags, item) {
  ...
  if (item instanceof resolveSeq.Scalar) {
    ...
    tagObj = match.find((t) => t.format === item.format) ||
...
class Anchors {
  static validAnchorNode(node) {
    return node instanceof resolveSeq.Scalar ||
      node instanceof resolveSeq.YAMLSeq || node instanceof resolveSeq.YAMLMap;
  }
...
function parseContents(doc, contents) {
  ...
  for (const node of contents) {
    if (node.valueRange) {
      ...
      if (
        body === undefined && comments.before.length > 0 && !doc.commentBefore
      ) {
  ...
  if (!body) {
    ...
    if (cb) {
      const cbNode = body instanceof resolveSeq.Collection && body.items[0]
        ? body.items[0]
        : body;
...
function resolveYamlDirective(doc, directive) {
  ...
  if (!documentOptions[version]) {
    ...
    const msg =
      `Document will be parsed as YAML ${v0} rather than YAML ${version}`;
  ...
  for (const directive of directives) {
    ...
    switch (name) {
      ...
      default:
        if (name) {
          const msg =
            `YAML only supports %TAG and %YAML directives, and not %${name}`;
          doc.warnings.push(new PlainValue$1.YAMLWarning(directive, msg));
        }
    }
...
class Document$2 {
  ...
  getDefaults() {
    return Document$2.defaults[this.version] ||
  ...
  listNonDefaultTags() {
    return listTagNames(this.contents).filter((t) =>
      t.indexOf(Schema$1.Schema.defaultPrefix) !== 0
    );
  }
...
class ExplorerBase {
  ...
  validateConfig() {
    ...
    config.searchPlaces.forEach((place) => {
      ...
      if (!loader) {
        throw new Error(
          `No loader specified for ${
            getExtensionDescription(place)
...
class Explorer extends _ExplorerBase$1.ExplorerBase {
  ...
  async searchFromDirectory(dir) {
    ...
    if (this.searchCache) {
      return (0, _cacheWrapper$1.cacheWrapper)(
        this.searchCache,
        absoluteDir,
        run,
      );
  ...
  const normalizedOptions = Object.assign(
    ...
    {},
    {
      loaders: Object.assign(
        Object.assign({}, defaults.loaders),
        options.loaders,
      ),
    },
  );
      ...
      switch (typeof vendor.pr) {
        ...
        case "object":
          if ("env" in vendor.pr) {
            ...
            exports.isPR = vendor.pr.env in env &&
