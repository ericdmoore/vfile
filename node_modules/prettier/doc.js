(function (global, factory) {
  ...
  var getBuiltIn$4 = function (namespace, method) {
    return arguments.length < 2
      ? aFunction(global$p[namespace])
      : global$p[namespace] && global$p[namespace][method];
  };
  ...
  var ordinaryToPrimitive$1 = function (input, pref) {
    ...
    if (
      ...
      !isObject$5(val = call$5(fn, input))
    ) {
      return val;
    }
    if (
      isCallable$6(fn = input.valueOf) && !isObject$5(val = call$5(fn, input))
    ) {
      return val;
    }
    if (
  ...
  var nativeWeakMap = isCallable$4(WeakMap$2) &&
  ...
  var objectKeysInternal = function (object, names) {
    ...
    var key;
    for (key in O) {
      !hasOwn$1(hiddenKeys$1, key) && hasOwn$1(O, key) && push$1(result, key);
    }
  ...
  var flattenIntoArray$1 = function (
    target,
    original,
    source,
    sourceLen,
  ...
  (function (module, exports) {
    ...
    createToken(
      ...
      "(?:".concat(src[t.NUMERICIDENTIFIER], "|").concat(
    ...
    createToken(
      ...
      "(?:".concat(src[t.NUMERICIDENTIFIERLOOSE], "|").concat(
        src[t.NONNUMERICIDENTIFIER],
        ")",
      ),
    ); // ## Pre-release Version
    ...
    createToken(
      "PRERELEASE",
      "(?:-(".concat(src[t.PRERELEASEIDENTIFIER], "(?:\\.").concat(
    ...
    createToken(
      "PRERELEASELOOSE",
      "(?:-?(".concat(src[t.PRERELEASEIDENTIFIERLOOSE], "(?:\\.").concat(
    ...
    createToken(
      "BUILD",
      "(?:\\+(".concat(src[t.BUILDIDENTIFIER], "(?:\\.").concat(
    ...
    createToken(
      "FULLPLAIN",
      "v?".concat(src[t.MAINVERSION]).concat(src[t.PRERELEASE], "?").concat(
        src[t.BUILD],
        "?",
      ),
    );
    ...
    createToken(
      "LOOSEPLAIN",
      "[v=\\s]*".concat(src[t.MAINVERSIONLOOSE]).concat(
    ...
    createToken(
      "COMPARATORTRIM",
      "(\\s*)".concat(src[t.GTLT], "\\s*(").concat(src[t.LOOSEPLAIN], "|")
    ...
    createToken(
      "HYPHENRANGE",
      "^\\s*(".concat(src[t.XRANGEPLAIN], ")") + "\\s+-\\s+" +
        "(".concat(src[t.XRANGEPLAIN], ")") + "\\s*$",
    );
    createToken(
      "HYPHENRANGELOOSE",
      "^\\s*(".concat(src[t.XRANGEPLAINLOOSE], ")") + "\\s+-\\s+" +
        "(".concat(src[t.XRANGEPLAINLOOSE], ")") + "\\s*$",
  ...
  class SemVer$1 {
    ...
    format() {
      this.version = "".concat(this.major, ".").concat(this.minor, ".").concat(
        this.patch,
      );
  ...
  var arrayify$1 = (object, keyName) =>
    Object.entries(object).map((_ref) => {
      let [key, value] = _ref;
  ...
  function getSupportInfo$1() {
    ...
    const options = arrayify(
      ...
      "name",
    ).filter((option) => filterSince(option) && filterDeprecated(option)).sort((
      a,
      b,
    ) => a.name === b.name ? 0 : a.name < b.name ? -1 : 1).map(mapInternal).map(
      (option) => {
        ...
        if (Array.isArray(option.default)) {
          option.default = option.default.length === 1
            ...
            : option.default.filter(filterSince).sort((info1, info2) =>
        ...
        if (Array.isArray(option.choices)) {
          option.choices = option.choices.filter((option) =>
            filterSince(option) && filterDeprecated(option)
          );
    ...
    function filterSince(object) {
      return showUnreleased || !("since" in object) ||
        object.since && semver.gte(version, object.since);
    }

    function filterDeprecated(object) {
      return showDeprecated || !("deprecated" in object) ||
        ...
        for (const value of language.parsers) {
          if (!existingValues.has(value)) {
            ...
            const plugin = plugins.find((plugin) =>
              plugin.parsers && plugin.parsers[value]
            );
  ...
  function getAlignmentSize(value, tabWidth) {
    let startIndex = arguments.length > 2 && arguments[2] !== undefined
      ? arguments[2]
      : 0;
    let size = 0;
  ...
  function getPreferredQuote(rawContent, preferredQuote) {
    ...
    if (
      ...
      const numPreferredQuotes =
        (rawContent.match(preferred.regex) || []).length;
      const numAlternateQuotes =
  ...
  function printString(raw, options) {
    ...
    const enclosingQuote =
      options.parser === "json" ||
        options.parser === "json5" && options.quoteProps === "preserve" &&
          !options.singleQuote
        ? '"'
        : options.__isInHtmlAttribute
        ? "'"
        : getPreferredQuote(rawContent, options.singleQuote ? "'" : '"').quote; // It might sound unnecessary to use `makeString` even if the string already
    ...
    return makeString(
      ...
      !(options.parser === "css" || options.parser === "less" ||
        options.parser === "scss" || options.__embeddedInHtml),
    );
  }
  /**
   * @param {string} rawContent
  ...
  function inferParserByLanguage(language, options) {
    ...
    const matched = languages.find((_ref) => {
      ...
      return Array.isArray(extensions) &&
        extensions.includes(".".concat(language));
    });
  ...
  function describeNodeForDebugging(node) {
    ...
    let nodeName = String(
      node.name || node.id && (typeof node.id === "object"
            ? node.id.name
            : node.id) || node.key && (typeof node.key === "object"
              ? node.key.name
              : node.key) || node.value && (typeof node.value === "object"
              ? ""
              : String(node.value)) || node.operator || "",
    );
  ...
  const isConcat$2 = (doc) =>
    Array.isArray(doc) || doc && doc.type === "concat";
  ...
  const isHardline = (doc, nextDoc) =>
    doc && doc.type === "line" && doc.hard && nextDoc &&
  ...
  function stripDocTrailingHardlineFromDoc(doc) {
    ...
    if (isConcat$2(doc) || doc.type === "fill") {
      ...
      return Array.isArray(doc)
        ? parts
        : Object.assign(Object.assign({}, doc), {}, {
          parts,
        });
    }
    ...
    switch (doc.type) {
      ...
      case "if-break": {
        const breakContents = stripDocTrailingHardlineFromDoc(
          doc.breakContents,
        );
  ...
  function cleanDocFn(doc) {
    switch (doc.type) {
      ...
      case "group":
        ...
        if (
          doc.contents.type === "group" && doc.contents.id === doc.id &&
    ...
    for (const part of getDocParts$2(doc)) {
      ...
      const [currentPart, ...restParts] = isConcat$2(part)
        ? getDocParts$2(part)
        : [part];

      if (
        typeof currentPart === "string" && typeof getLast$1(parts) === "string"
      ) {
  ...
  function normalizeParts(parts) {
    ...
    while (restParts.length > 0) {
      ...
      if (
        newParts.length > 0 && typeof getLast$1(newParts) === "string" &&
        typeof part === "string"
      ) {
  ...
  function trim(out) {
    ...
    while (
      out.length > 0 && typeof getLast(out) === "string" &&
      /^[\t ]*$/.test(getLast(out))
    ) {
      ...
      if (typeof doc === "string") {
        ...
        switch (doc.type) {
          ...
          case "group": {
            ...
            cmds.push([
              ind,
              groupMode, // The most expanded state takes up the least space on the current line.
          ...
          case "group":
            switch (mode) {
              ...
              case MODE_BREAK: {
                ...
                if (
                  !doc.break && fits(next, cmds, rem, options, hasLineSuffix)
                ) {
                  cmds.push(next);
                } else {
  ...
  function printDocToDebug(doc) {
    ...
    function printDoc(doc, index, parentParts) {
      ...
      if (doc.type === "line") {
        const withBreakParent = Array.isArray(parentParts) &&
          parentParts[index + 1] &&
      ...
      if (doc.type === "break-parent") {
        const afterHardline = Array.isArray(parentParts) &&
          parentParts[index - 1] && parentParts[index - 1].type === "line" &&
      ...
      if (doc.type === "if-break") {
        return "ifBreak(" + printDoc(doc.breakContents) + (doc.flatContents
          ? ", " + printDoc(doc.flatContents)
          : "") +
          (doc.groupId
            ? (!doc.flatContents ? ', ""' : "") +
              ", { groupId: ".concat(printGroupId(doc.groupId), " }")
            : "") +
          ")";
      }
      ...
      if (doc.type === "indent-if-break") {
        ...
        const options = optionsParts.length > 0
          ? ", { ".concat(optionsParts.join(", "), " }")
          : "";
        return "indentIfBreak(".concat(printDoc(doc.contents)).concat(
          options,
          ")",
        );
      }
      ...
      if (doc.type === "group") {
        ...
        if (doc.expandedStates) {
          return "conditionalGroup([".concat(
            doc.expandedStates.map((part) => printDoc(part)).join(","),
            "]",
          ).concat(options, ")");
        }
      ...
      if (doc.type === "fill") {
        return "fill([".concat(
          doc.parts.map((part) => printDoc(part)).join(", "),
          "])",
        );
      }
      ...
      if (doc.type === "label") {
        return "label(".concat(JSON.stringify(doc.label), ", ").concat(
    ...
    function printGroupId(id) {
      ...
      for (let counter = 0;; counter++) {
        ...
        if (!usedKeysForSymbols.has(key)) {
          ...
          return printedSymbols[id] = "Symbol.for(".concat(
            JSON.stringify(key),
            ")",
          );
        }
