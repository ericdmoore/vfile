...
var pleaseUpgradeNode = function pleaseUpgradeNode(pkg, opts) {
  ...
  if (semverCompare(currentVersion, requiredVersion) === -1) {
    if (opts.message) {
      ...
      console.error(
        pkg.name + " requires at least version " + requiredVersion +
...
// Nashorn ~ JDK8 bug
var NASHORN_BUG = getOwnPropertyDescriptor$1 &&
...
var functionUncurryThis = bind$3
  ? function (fn) {
...
var isSymbol$6 = USE_SYMBOL_AS_UID$1
  ? function (it) {
    return typeof it == "symbol";
  }
  : function (it) {
    var $Symbol = getBuiltIn$3("Symbol");
    return isCallable$8($Symbol) &&
...
var ordinaryToPrimitive$1 = function (input, pref) {
  var fn, val;
  if (
    pref === "string" && isCallable$6(fn = input.toString) &&
    !isObject$c(val = call$5(fn, input))
  ) {
    return val;
  }
  if (
    isCallable$6(fn = input.valueOf) && !isObject$c(val = call$5(fn, input))
  ) {
    return val;
  }
  if (
...
var createWellKnownSymbol = USE_SYMBOL_AS_UID
  ? Symbol$7
  : Symbol$7 && Symbol$7.withoutSetter || uid$1;
...
objectDefineProperty.f = DESCRIPTORS$3
  ...
  : function defineProperty(O, P, Attributes) {
    ...
    if ("get" in Attributes || "set" in Attributes) {throw TypeError$5(
...
var nativeWeakMap = isCallable$4(WeakMap$4) &&
  /native code/.test(inspectSource$2(WeakMap$4));
...
(redefine$1.exports = function (O, key, value, options) {
  ...
  return isCallable$3(this) && getInternalState(this).source ||
...
objectGetOwnPropertyNames.f = Object.getOwnPropertyNames ||
...
convert$1.rgb.ansi256 = function (args) {
  ...
  const ansi = 16 + 36 * Math.round(r / 255 * 5) + 6 * Math.round(g / 255 * 5) +
    Math.round(b / 255 * 5);
  return ansi;
};
...
if (
  ...
  hasFlag("color") || hasFlag("colors") || hasFlag("color=true") ||
...
function supportsColor(haveStream, streamIsTTY) {
  ...
  if (
    hasFlag("color=16m") || hasFlag("color=full") || hasFlag("color=truecolor")
  ) {
    return 3;
  }
...
const stringEncaseCRLFWithFirstIndex$1 = (string, prefix, postfix, index) => {
  ...
  do {
    ...
    returnValue +=
      string.substr(endIndex, (gotCR ? index - 1 : index) - endIndex) + prefix +
      (gotCR ? "\r\n" : "\n") + postfix;
    endIndex = index + 1;
...
const applyOptions = (object, options = {}) => {
  if (
    options.level &&
    !(Number.isInteger(options.level) && options.level >= 0 &&
      options.level <= 3)
  ) {
...
var flattenIntoArray$2 = function (
  target,
  original,
  source,
  sourceLen,
    ...
    if (sourceIndex in source) {
      ...
      if (depth > 0 && isArray$c(element)) {
        ...
        if (targetIndex >= 0x1FFFFFFFFFFFFF) {
          throw TypeError$3("Exceed the acceptable array length");
        }
...
var arraySpeciesConstructor$1 = function (originalArray) {
  ...
  if (isArray$b(originalArray)) {
    ...
    if (isConstructor(C) && (C === Array$1 || isArray$b(C.prototype))) {
      C = undefined;
...
path$c.removeLeadingDotSegment = path$c.escape = path$c.makeAbsolute = path$c
  .unixify = void 0;
...
(function (exports) {
  ...
  exports.escapeNode = (block, n = 0, type) => {
    ...
    if (
      type && node.type === type || node.type === "open" ||
      node.type === "close"
    ) {
...
const fillLetters = (start, end, step = 1, options = {}) => {
  if (
    !isNumber$1(start) && start.length > 1 || !isNumber$1(end) && end.length > 1
  ) {
...
const expand$1 = (ast, options = {}) => {
  ...
  let walk = (node, parent = {}) => {
    ...
    if (
      node.type === "brace" && node.invalid !== true && node.nodes.length === 2
    ) {
      q.push(append(q.pop(), ["{}"]));
      return;
    }
...
const parse$4 = (input, options = {}) => {
  ...
  let opts = options || {};
  let max = typeof opts.maxLength === "number"
  ...
  while (index < length) {
    ...
    if (
      value === CHAR_DOUBLE_QUOTE || value === CHAR_SINGLE_QUOTE ||
      value === CHAR_BACKTICK
    ) {
      let open = value;
      let next;
  ...
  while (index < length) {
    ...
    if (braceEscaped === true || code === CHAR_LEFT_CURLY_BRACE) {
      ...
      while (eos() !== true && (code = advance())) {
        ...
        if (
          braceEscaped !== true && code === CHAR_DOT &&
    ...
    if (opts.noext !== true) {
      const isExtglobChar = code === CHAR_PLUS || code === CHAR_AT ||
        code === CHAR_ASTERISK || code === CHAR_QUESTION_MARK ||
    ...
    if (
      opts.nonegate !== true && code === CHAR_EXCLAMATION_MARK &&
      index === start
    ) {
...
const parse$2 = (input, options) => {
  ...
  const push = (tok) => {
    if (prev.type === "globstar") {
      const isBrace = state.braces > 0 &&
        (tok.type === "comma" || tok.type === "brace");
      const isExtglob = tok.extglob === true ||
        extglobs.length && (tok.type === "pipe" || tok.type === "paren");

      if (
        tok.type !== "slash" && tok.type !== "paren" && !isBrace && !isExtglob
      ) {
  ...
  const extglobClose = (token) => {
    ...
    if (token.type === "negate") {
      ...
      if (
        token.inner.includes("*") && (rest = remaining()) &&
        /^\.[^\\/.]+$/.test(rest)
      ) {
  ...
  while (!eos()) {
    ...
    if (value === ".") {
      ...
      if (
        state.braces + state.parens === 0 && prev.type !== "bos" &&
        prev.type !== "slash"
      ) {
        push({
          type: "text",
      ...
      if (
        prev &&
          (prev.type === "bracket" || prev.type === "paren" ||
            prev.type === "brace") || state.parens > 0
      ) {
        push({
          type: "plus",
          value,
    ...
    if (prev.type === "star") {
      ...
      const isExtglob = extglobs.length &&
      ...
      if (
        prior.type === "slash" && prior.prev.type !== "bos" && !afterStar &&
        eos()
      ) {
    ...
    if (
      prev && (prev.type === "bracket" || prev.type === "paren") &&
      opts.regex === true
    ) {
      token.output = value;
      push(token);
    ...
    if (
      state.index === state.start || prev.type === "slash" ||
      prev.type === "dot"
    ) {
      if (prev.type === "dot") {
...
picomatch$2.matchBase = (
  input,
  glob,
  options,
  posix = utils$c.isWindows(options),
) => {
  const regex = glob instanceof RegExp
    ? glob
    : picomatch$2.makeRe(glob, options);
...
picomatch$2.isMatch = (str, patterns, options) =>
  picomatch$2(patterns, options)(str);
/**
...
micromatch$1.isMatch = (str, patterns, options) =>
...
pattern$1.matchAny = pattern$1.convertPatternsToRe = pattern$1.makeRe =
  pattern$1.getPatternParts = pattern$1.expandBraceExpansion = pattern$1
    .expandPatternsWithBraceExpansion = pattern$1
      .isAffectDepthOfReadingPattern = pattern$1.endsWithSlashGlobStar =
        pattern$1.hasGlobStar = pattern$1.getBaseDirectory = pattern$1
          .isPatternRelatedToParentDirectory = pattern$1
            .getPatternsOutsideCurrentDirectory = pattern$1
              .getPatternsInsideCurrentDirectory = pattern$1
                .getPositivePatterns = pattern$1.getNegativePatterns = pattern$1
                  .isPositivePattern = pattern$1.isNegativePattern = pattern$1
                    .convertToNegativePattern = pattern$1
                      .convertToPositivePattern = pattern$1.isDynamicPattern =
                        pattern$1.isStaticPattern = void 0;
...
pattern$1.getPatternsOutsideCurrentDirectory =
...
utils$k.string = utils$k.stream = utils$k.pattern = utils$k.path = utils$k.fs =
  utils$k.errno = utils$k.array = void 0;
...
tasks.convertPatternGroupToTask = tasks.convertPatternGroupsToTasks = tasks
  .groupPatternsByBaseDirectory = tasks.getNegativePatternsAsPositive = tasks
    .getPositivePatterns = tasks.convertPatternsToTasks = tasks.generate =
      void 0;
...
function generate(patterns, settings) {
  ...
  const staticPatterns = positivePatterns.filter((pattern) =>
    utils$a.pattern.isStaticPattern(pattern, settings)
  );
  const dynamicPatterns = positivePatterns.filter((pattern) =>
    utils$a.pattern.isDynamicPattern(pattern, settings)
  );
...
function convertPatternsToTasks(positive, negative, dynamic) {
  const tasks = [];
  const patternsOutsideCurrentDirectory = utils$a.pattern
    .getPatternsOutsideCurrentDirectory(positive);
  const patternsInsideCurrentDirectory = utils$a.pattern
...
function convertPatternGroupToTask(base, positive, negative, dynamic) {
  return {
    ...
    patterns: [].concat(
      positive,
      negative.map(utils$a.pattern.convertToNegativePattern),
    ),
  };
}
...
var queueMicrotask_1 = typeof queueMicrotask === "function"
  ? queueMicrotask.bind(typeof window !== "undefined" ? window : global) // reuse resolved promise, and allocate it lazily
  : (cb) =>
...
constants.IS_SUPPORT_READDIR_WITH_FILE_TYPES = IS_MATCHED_BY_MAJOR ||
...
function readdirWithFileTypes$1(directory, settings, callback) {
  settings.fs.readdir(directory, {
    ...
    const entries = dirents.map((dirent) => ({
      dirent,
      name: dirent.name,
      path: common$5.joinPathSegments(
        directory,
        dirent.name,
...
function readdir$1(directory, settings, callback) {
  settings.fs.readdir(directory, (readdirError, names) => {
    ...
    const tasks = names.map((name) => {
      const path = common$5.joinPathSegments(
        directory,
        name,
        settings.pathSegmentSeparator,
      );
...
function readdirWithFileTypes(directory, settings) {
  ...
  return dirents.map((dirent) => {
    const entry = {
      dirent,
      name: dirent.name,
      path: common$4.joinPathSegments(
        directory,
        dirent.name,
        settings.pathSegmentSeparator,
      ),
...
function readdir(directory, settings) {
  ...
  return names.map((name) => {
    const entryPath = common$4.joinPathSegments(
      directory,
      name,
      settings.pathSegmentSeparator,
    );
...
common$3.joinPathSegments = common$3.replacePathSegmentSeparator = common$3
  .isAppliedFilter = common$3.isFatalError = void 0;
...
class Reader$1 {
  constructor(_root, _settings) {
    ...
    this._root = common$2.replacePathSegmentSeparator(
      _root,
...
function walk(directory, optionsOrSettingsOrCallback, callback) {
  if (typeof optionsOrSettingsOrCallback === "function") {
    new async_1$1.default(directory, getSettings()).read(
      optionsOrSettingsOrCallback,
    );
    return;
  }

  new async_1$1.default(directory, getSettings(optionsOrSettingsOrCallback))
    .read(callback);
}
...
class Reader {
  ...
  _isFatalError(error) {
    return !utils$6.errno.isEnoentCodeError(error) &&
...
class ReaderStream extends reader_1$1.default {
  ...
  static(patterns, options) {
    ...
    stream._write = (index, _enc, done) => {
      return this._getEntry(filepaths[index], patterns[index], options).then(
        (entry) => {
  ...
  _getEntry(filepath, pattern, options) {
    return this._getStat(filepath).then((stats) =>
      this._makeEntry(stats, pattern)
    ).catch((error) => {
...
class Matcher {
  ...
  _fillStorage() {
     ...
     */
    const patterns = utils$5.pattern.expandPatternsWithBraceExpansion(
      this._patterns,
    );
  ...
  _getPatternSegments(pattern) {
    const parts = utils$5.pattern.getPatternParts(
      pattern,
  ...
  _splitSegmentsIntoSections(segments) {
    return utils$5.array.splitWhen(
      segments,
      (segment) =>
        segment.dynamic && utils$5.pattern.hasGlobStar(segment.pattern),
    );
  }
}
...
class DeepFilter {
  ...
  _getNegativePatternsRe(patterns) {
    const affectDepthOfReadingPatterns = patterns.filter(
      utils$4.pattern.isAffectDepthOfReadingPattern,
    );
    return utils$4.pattern.convertPatternsToRe(
...
class EntryFilter {
  ...
  getFilter(positive, negative) {
    const positiveRe = utils$3.pattern.convertPatternsToRe(
      positive,
      this._micromatchOptions,
    );
    const negativeRe = utils$3.pattern.convertPatternsToRe(
      negative,
      this._micromatchOptions,
    );
  ...
  _filter(entry, positiveRe, negativeRe) {
    ...
    const isMatched = this._isMatchToPatterns(filepath, positiveRe) &&
...
class ErrorFilter {
  ...
  _isNonFatalError(error) {
    return utils$2.errno.isEnoentCodeError(error) ||
      this._settings.suppressErrors;
  }
}
...
class ProviderStream extends provider_1$1.default {
  ...
  read(task) {
    ...
    source.once("error", (error) => destination.emit("error", error)).on(
      "data",
      (entry) => destination.emit("data", options.transform(entry)),
...
async function* expandPatternsInternal(context) {
    ...
    if (stat) {
      if (stat.isFile()) {
        ...
        */
        ...
        entries.push({
          type: "dir",
          glob: escapePathForGlob(fixWindowsSlashes$1(relativePath)) + "/" +
  ...
  function getSupportedFilesGlob() {
    if (!supportedFilesGlob) {
      const extensions = context.languages.flatMap((lang) =>
        lang.extensions || []
      );
      const filenames = context.languages.flatMap((lang) =>
        lang.filenames || []
      );
      supportedFilesGlob = `**/{${[
...
function containsIgnoredPathSegment(absolutePath, cwd, ignoredDirectories) {
  return path$2.relative(cwd, absolutePath).split(path$2.sep).some((dir) =>
...
var isArrayIteratorMethod$1 = function (it) {
  return it !== undefined &&
    (Iterators$1.Array === it || ArrayPrototype$1[ITERATOR$1] === it);
};
...
var getIterator$1 = function (argument, usingIterator) {
  ...
  if (aCallable(iteratorMethod)) {
    return anObject$4(call$2(iteratorMethod, argument));
  }
...
var dashify$2 = (str, options) => {
  ...
  return str.trim().replace(/([a-z])([A-Z])/g, "$1-$2").replace(
    /\W/g,
    (m) => /[À-ž]/.test(m) ? m : "-",
  ).replace(/^-+|-+$/g, "").replace(
    /-{2,}/g,
    (m) => options && options.condense ? "-" : m,
  ).toLowerCase();
};
...
var minimist$3 = function (args, opts) {
  ...
  function argDefined(key, arg) {
    return flags.allBools && /^--[^=]+$/.test(arg) || flags.strings[key] ||
      flags.bools[key] || aliases[key];
  }
  ...
  function setKey(obj, keys, value) {
    ...
    if (key === "__proto__") return;
    if (
      o === Object.prototype || o === Number.prototype || o === String.prototype
    ) {
      o = {};
    }
    ...
    if (
      o[key] === undefined || flags.bools[key] || typeof o[key] === "boolean"
    ) {
      o[key] = value;
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (/^--.+=/.test(arg)) {
      ...
      if (
        next !== undefined && !/^-/.test(next) && !flags.bools[key] &&
        !flags.allBools && (aliases[key] ? !aliasIsBoolean(key) : true)
      ) {
        setArg(key, next, arg);
        i++;
      ...
      if (!broken && key !== "-") {
        if (
          ...
          !flags.bools[key] && (aliases[key] ? !aliasIsBoolean(key) : true)
        ) {
...
var freeGlobal$1 = typeof global == "object" && global &&
...
var isArguments$3 = baseIsArguments(function () {
  ...
  ? baseIsArguments
  : function (value) {
    return isObjectLike$3(value) && hasOwnProperty$9.call(value, "callee") &&
...
function baseIsTypedArray$1(value) {
  return isObjectLike$2(value) && isLength$2(value.length) &&
...
function arrayLikeKeys$1(value, inherited) {
  ...
  for (var key in value) {
    if (
      (inherited || hasOwnProperty$8.call(value, key)) && !(skipIndexes && ( // Safari 9 has enumerable `arguments.length` in strict mode.
...
var reIsNative = RegExp(
  "^" +
    funcToString.call(hasOwnProperty$6).replace(reRegExpChar, "\\$&").replace(
...
function equalObjects$1(object, other, bitmask, customizer, equalFunc, stack) {
  ...
  if (result && !skipCtor) {
    ...
    if (
      objCtor != othCtor && "constructor" in object && "constructor" in other &&
...
function baseIsEqual$2(value, other, bitmask, customizer, stack) {
  ...
  if (
    ...
    !isObjectLike$1(value) && !isObjectLike$1(other)
  ) {
...
function isKey$3(value, object) {
  ...
  if (
    type == "number" || type == "symbol" || type == "boolean" ||
...
var createMinimistOptions$2 = function createMinimistOptions(detailedOptions) {
  const [boolean, string] = partition(detailedOptions, ({
    type,
  }) => type === "boolean").map((detailedOptions) =>
    detailedOptions.flatMap(({
      name,
      alias,
    }) => alias ? [name, alias] : [name])
  );
  const defaults = Object.fromEntries(
    detailedOptions.filter((option) =>
      !option.deprecated &&
      (!option.forwardToApi || option.name === "plugin" ||
...
function parseArgsToOptions(context, overrideDefaults) {
  ...
  return getOptions(
    optionsNormalizer.normalizeCliOptions(
      minimist$1(context.rawArguments, {
...
function applyConfigPrecedence(context, options) {
  try {
    switch (context.argv["config-precedence"]) {
      ...
      case "file-override":
        return Object.assign(
          Object.assign({}, parseArgsToOptions(context)),
          options,
        );
...
_base$6 = _interopRequireDefault$7(base); /*istanbul ignore end*/
/*istanbul ignore start*/
...
var /*istanbul ignore start*/
_base$5 = _interopRequireDefault$6(base); /*istanbul ignore end*/

var /*istanbul ignore start*/
_params$1 = params; /*istanbul ignore end*/
/*istanbul ignore start*/
...
var /*istanbul ignore start*/
_base$4 = _interopRequireDefault$5(base); /*istanbul ignore end*/

var /*istanbul ignore start*/
_params = params; /*istanbul ignore end*/
/*istanbul ignore start*/
...
var /*istanbul ignore start*/
_base$3 = _interopRequireDefault$4(base); /*istanbul ignore end*/
/*istanbul ignore start*/
...
var /*istanbul ignore start*/
_base$2 = _interopRequireDefault$3(base); /*istanbul ignore end*/
/*istanbul ignore start*/
...
var /*istanbul ignore start*/
_base$1 = _interopRequireDefault$2(base); /*istanbul ignore end*/

var /*istanbul ignore start*/
_line$1 = line; /*istanbul ignore end*/
/*istanbul ignore start*/
...
var /*istanbul ignore start*/
_base = _interopRequireDefault$1(base); /*istanbul ignore end*/
/*istanbul ignore start*/
...
var /*istanbul ignore start*/
_parse$1 = parse; /*istanbul ignore end*/

var /*istanbul ignore start*/
_distanceIterator = _interopRequireDefault(
  distanceIterator,
); /*istanbul ignore end*/
/*istanbul ignore start*/
  ...
  for (var i = 0; i < hunks.length; i++) {
    ...
    var iterator =
      ...
      (
        ...
        /*istanbul ignore start*/
        0,
          _distanceIterator/*istanbul ignore end*/
          [
            /*istanbul ignore start*/
            "default"
            /*istanbul ignore end*/
...
var /*istanbul ignore start*/
_line = line; /*istanbul ignore end*/
/*istanbul ignore start*/
...
var /*istanbul ignore start*/
_create = create$1; /*istanbul ignore end*/
var /*istanbul ignore start*/
_parse = parse; /*istanbul ignore end*/
var /*istanbul ignore start*/
_array = array; /*istanbul ignore end*/
/*istanbul ignore start*/
...
(function (exports) {
  ...
  var /*istanbul ignore start*/
  _base = _interopRequireDefault(base); /*istanbul ignore end*/

  var /*istanbul ignore start*/
  _character = character; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _word = word; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _line = line; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _sentence = sentence; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _css = css; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _json = json; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _array = array$1; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _apply = apply$2; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _parse = parse; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _merge = merge$1; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _create = create$1; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _dmp = dmp; /*istanbul ignore end*/
  var /*istanbul ignore start*/
  _xml = xml; /*istanbul ignore end*/
  /*istanbul ignore start*/
...
function handleError(context, filename, error, printedFilename) {
  if (error instanceof errors.UndefinedParserError) {
    ...
    /* istanbul ignore next */
    if (
      (context.argv.write || context.argv["ignore-unknown"]) && printedFilename
    ) {
...
function format$1(context, input, opt) {
  ...
  if (context.argv["debug-print-comments"]) {
    return {
      formatted: prettier$4.format(
        JSON.stringify(prettier$4.formatWithCursor(input, opt).comments || []),
        {
          parser: "json",
        },
      ),
...
async function formatStdin$1(context) {
  const filepath = context.argv["stdin-filepath"]
    ? path$1.resolve(process.cwd(), context.argv["stdin-filepath"])
    : process.cwd();
  ...
  const relativeFilepath = context.argv["ignore-path"]
    ? path$1.relative(path$1.dirname(context.argv["ignore-path"]), filepath)
    : path$1.relative(process.cwd(), filepath);

  try {
    const input = await getStdin();

    if (
      relativeFilepath && ignorer.ignores(fixWindowsSlashes(relativeFilepath))
    ) {
      writeOutput(context, {
        formatted: input,
      });
...
async function formatFiles$1(context) {
  ...
  for await (const pathOrError of expandPatterns(context)) {
    ...
    const ignoreFilename = context.argv["ignore-path"]
      ? path$1.relative(path$1.dirname(context.argv["ignore-path"]), filename)
      : filename;
    const fileIgnored = ignorer.ignores(fixWindowsSlashes(ignoreFilename));

    if (
      fileIgnored &&
      (context.argv["debug-check"] || context.argv.write ||
    ...
    const options = Object.assign(
      Object.assign({}, await getOptionsForFile(context, filename)),
      {},
      {
        filepath: filename,
      },
  ...
  function createInstance(options) {
    ...
    function outdent(stringsOrOptions) {
      ...
      if (isTemplateStringsArray(stringsOrOptions)) {
        ...
        var firstInterpolatedValueSetsIndentationLevel =
          (values[0] === outdent || values[0] === defaultOutdent) &&
          reOnlyWhitespaceWithAtLeastOneNewline.test(strings[0]) &&
          reStartsWithNewlineOrIsEmpty.test(strings[1]); // Perform outdentation
        ...
        var rendered = concatStringsAndValues(
          renderedArray,
...
const categoryOrder = [
  coreOptions$1.CATEGORY_OUTPUT,
  coreOptions$1.CATEGORY_FORMAT,
  coreOptions$1.CATEGORY_CONFIG,
  coreOptions$1.CATEGORY_EDITOR,
  coreOptions$1.CATEGORY_OTHER,
];
/**
 * {
 *   [optionName]: {
...
function normalizeDetailedOptionMap$2(detailedOptionMap) {
  return Object.fromEntries(
    Object.entries(detailedOptionMap).sort(([leftName], [rightName]) =>
...
class Context$1 {
  constructor({
    ...
    } = parseArgvWithoutPlugins$1(rawArguments, logger, [
      "plugin",
      "plugin-search-dir",
    ]);
...
function getContextOptions(plugins, pluginSearchDirs) {
  ...
  const detailedOptionMap = normalizeDetailedOptionMap$1(
    Object.assign(
      Object.assign({}, createDetailedOptionMap$1(supportOptions)),
      constant$1.options,
    ),
  );
  const detailedOptions = arrayify(detailedOptionMap, "name");
  const apiDefaultOptions = Object.assign(
...
var objectDefineProperties = DESCRIPTORS
  ...
  : function defineProperties(O, Properties) {
    ...
    var index = 0;
    var key;
    while (length > index) {definePropertyModule$1.f(
        O,
        key = keys[index++],
        props[key],
      );}
...
const postProcess = (input, options) => {
  ...
  return input.replace(
    ...
    (_, identifier) => identifier.toLocaleUpperCase(options.locale),
    ...
    case "choice":
      return `<${
        option.choices.filter((choice) =>
          !choice.deprecated && choice.since !== null
...
function getOptionsWithOpposites(options) {
  ...
  const optionsWithOpposites = options.map(
    (option) => [
...
function createUsage$1(context) {
  const options = getOptionsWithOpposites(context.detailedOptions).filter( // remove unnecessary option (e.g. `semi`, `color`, etc.), which is only used for --help <flag>
    (option) =>
      !(option.type === "boolean" && option.oppositeDescription &&
  ...
  const restCategories = Object.keys(groupedOptions).filter((category) =>
    !constant.categoryOrder.includes(category)
  );
  const allCategories = [
    ...firstCategories,
    ...restCategories,
  ...
  const optionsUsage = allCategories.map((category) => {
    const categoryOptions = groupedOptions[category].map((option) =>
...
function createDetailedUsage$1(context, flag) {
  ...
  const pluginDefaults =
    option.pluginDefaults && Object.keys(option.pluginDefaults).length > 0
      ? `\nPlugin defaults:${
        Object.entries(option.pluginDefaults).map(([key, value]) =>
...
async function logFileInfoOrDie(context) {
  ...
  context.logger.log(
    prettier$2.format(
      stringify$1(
        await prettier$2.getFileInfo(context.argv["file-info"], options),
      ),
      {
        parser: "json",
      },
...
async function run(rawArguments) {
  ...
  try {
    const logLevel =
      core.parseArgvWithoutPlugins(rawArguments, logger, "loglevel").loglevel;
