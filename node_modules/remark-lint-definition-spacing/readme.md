...
[`remark-lint`][mono] rule to warn when consecutive whitespace is used in a
definition label.
...
You can use this package to check that the labels used in definitions do not use
...
Definitions and references are matched together by collapsing white space. Using
more white space in labels might incorrectly indicate that they are of
importance. Due to this, it’s recommended to use one space (or a line ending if
needed) and turn this rule on.
