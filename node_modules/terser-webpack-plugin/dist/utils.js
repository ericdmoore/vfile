...
function throttleAll(limit, tasks) {
  ...
  if (
    !Array.isArray(tasks) || !tasks.every((task) => typeof task === `function`)
  ) {
    throw new TypeError(
...
async function terserMinify(
  ...
  const buildComments = (terserOptions, extractedComments) => {
    ...
    if (typeof extractComments === "boolean" && extractComments) {
      ...
      typeof extractComments === "string" || extractComments instanceof RegExp
    ) {
    ...
    return (astNode, comment) => {
      if (
        ...
        const commentText = comment.type === "comment2"
          ? `/*${comment.value}*/`
          : `//${comment.value}`; // Don't include duplicate comments
  ...
  const buildTerserOptions = (terserOptions = {}) => {
    ...
    return {
      ...terserOptions,
      compress: typeof terserOptions.compress === "boolean"
      ...
      mangle: terserOptions.mangle == null
        ? true
...
async function uglifyJsMinify(
  ...
  const buildUglifyJsOptions = (uglifyJsOptions = {}) => {
    ...
    return {
      ...
      compress: typeof uglifyJsOptions.compress === "boolean"
        ? uglifyJsOptions.compress
        : { ...uglifyJsOptions.compress },
      mangle: uglifyJsOptions.mangle == null ? true
...
async function swcMinify(input, sourceMap, minimizerOptions) {
  ...
  const buildSwcOptions = (swcOptions = {}) => {
    ...
    return {
      ...swcOptions,
      compress: typeof swcOptions.compress === "boolean"
        ? swcOptions.compress
        : { ...swcOptions.compress },
      mangle: swcOptions.mangle == null
        ? true
