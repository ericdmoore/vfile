...
class TerserPlugin {
      ...
      if (original && original.source && requestShortener) {
        builtError = new Error(
          `${file} from Terser plugin\n${error.message} [${
            requestShortener.shorten(original.source)
  ...
  async optimize(compiler, compilation, assets, optimizeOptions) {
    ...
    const assetsForMinify = await Promise.all(
      Object.keys(assets).filter((name) => {
        ...
        if (
          !compiler.webpack.ModuleFilenameHelpers.matchObject.bind( // eslint-disable-next-line no-undefined
            undefined,
            this.options,
          )(name)
        ) {
      ...
      scheduledTasks.push(async () => {
        ...
        if (!output) {
          ...
          try {
            ...
            const hasSourceMap = inputSourceMap &&
          ...
          if (output.extractedComments && output.extractedComments.length > 0) {
            ...
            if (
              ...
              banner =
                ...
                `For license information please see ${
                  path.relative(path.dirname(name), output.commentsFilename)
                    .replace(/\\/g, "/")
                }`;
            ...
            const extractedCommentsString = output.extractedComments.sort()
              .join("\n\n");
    ...
    await Array.from(allExtractedComments).sort().reduce(
      ...
      async (previousPromise, [from, value]) => {
        ...
        if (previous && previous.commentsFilename === commentsFilename) {
          ...
          const eTag = [prevSource, extractedCommentsSource].map((item) =>
            cache.getLazyHashedEtag(item)
  ...
  static getEcmaVersion(environment) {
    // ES 6th
    if (
      environment.arrowFunction || environment.const ||
      environment.destructuring || environment.forOf || environment.module
    ) {
      return 2015;
    } // ES 11th
  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap(pluginName, (compilation) => {
      const hooks = compiler.webpack.javascript.JavascriptModulesPlugin
