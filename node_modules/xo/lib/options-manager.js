...
import { flow, mergeWith, pick } from "lodash-es";
...
const resolveFrom = (moduleId, fromDirectory = process.cwd()) =>
  resolveModule(moduleId, path.join(fromDirectory, "__placeholder__.js"));
...
const resolveLocalConfig = (name) =>
  resolveModule(normalizePackageName(name, "eslint-config"), import.meta.url);
...
const isTypescript = (file) =>
  TYPESCRIPT_EXTENSION.includes(path.extname(file).slice(1));
...
const mergeWithFileConfig = async (options) => {
  ...
  const { config: xoOptions, filepath: xoConfigPath } =
    (await configExplorer.search(searchPath)) || {};
  const { config: enginesOptions } =
    (await pkgConfigExplorer.search(searchPath)) || {};

  options = normalizeOptions({
    ...xoOptions,
    ...(enginesOptions && enginesOptions.node &&
        semver.validRange(enginesOptions.node)
      ? { nodeVersion: enginesOptions.node }
      : {}),
    ...options,
  });
  options.extensions = [...DEFAULT_EXTENSION, ...(options.extensions || [])];
  options.ignores = getIgnores(options);
  options.cwd = xoConfigPath && path.dirname(xoConfigPath) !== options.cwd
    ? path.resolve(options.cwd, path.dirname(xoConfigPath))
    : options.cwd;
  ...
  const prettierOptions = options.prettier
    ? await prettier.resolveConfig(searchPath, { editorconfig: true }) || {}
    : {};
...
const handleTSConfig = async (options) => {
  ...
  const { project: tsConfigProjectPath, tsconfigRootDir } =
  ...
  const hasMatch =
    options.tsConfig && !options.tsConfig.include && !options.tsConfig.files
      ? true
      : micromatch.contains(options.filePath, [
        ...(options.tsConfig && Array.isArray(options.tsConfig.include)
          ? options.tsConfig.include
          : []),
        ...(options.tsConfig && Array.isArray(options.tsConfig.files)
  ...
  if (!hasMatch) {
    ...
    options.tsConfig = options.tsConfigPath
      ? { extends: options.tsConfigPath }
      : TSCONFIG_DEFAULTS;
    options.tsConfigHash = new MurmurHash3(stringify(options.tsConfig))
      .result();
...
const normalizeSpaces = (options) =>
  typeof options.space === "number" ? options.space : 2;
...
const toValueMap = (array, value = true) =>
  Object.fromEntries(array.map((item) => [item, value]));

const buildESLintConfig = (options) =>
  (config) => {
    if (options.rules) {
  ...
  (config) => {
    ...
    for (const [rule, ruleConfig] of Object.entries(ENGINE_RULES)) {
      for (const minVersion of Object.keys(ruleConfig).sort(semver.rcompare)) {
        if (
          !options.nodeVersion ||
          semver.intersects(options.nodeVersion, `<${minVersion}`)
        ) {
    ...
    if (options.nodeVersion) {
      config.baseConfig.rules["node/no-unsupported-features/es-builtins"] = [
        "error",
      ...
      config.baseConfig.rules["node/no-unsupported-features/node-builtins"] = [
        "error",
        { version: options.nodeVersion },
      ];
    }
    ...
    if (options.space && !options.prettier) {
      ...
      if (
        options.cwd && resolveFrom.silent("eslint-plugin-react", options.cwd)
      ) {
    ...
    config.baseConfig.settings["import/resolver"] = gatherImportResolvers(
      options,
    );
...
const buildExtendsConfig = (options) =>
  (config) => {
...
const buildPrettierConfig = (options, prettierConfig) =>
  (config) => {
    if (options.prettier) {
      ...
      config.baseConfig.rules["prettier/prettier"] = [
        "error",
        mergeWithPrettierConfig(options, prettierConfig),
      ];
    }
...
const mergeWithPrettierConfig = (options, prettierOptions) => {
  if (
    (options.semicolon === true && prettierOptions.semi === false) ||
    (options.semicolon === false && prettierOptions.semi === true)
  ) {
    throw new Error(
      `The Prettier config \`semi\` is ${prettierOptions.semi} while XO \`semicolon\` is ${options.semicolon}`,
    );
  }

  if (
    ((options.space === true || typeof options.space === "number") &&
      prettierOptions.useTabs === true) ||
    ((options.space === false) && prettierOptions.useTabs === false)
  ) {
    throw new Error(
      `The Prettier config \`useTabs\` is ${prettierOptions.useTabs} while XO \`space\` is ${options.space}`,
    );
  }
  ...
  if (
    ...
    options.space !== prettierOptions.tabWidth
  ) {
    throw new Error(
      `The Prettier config \`tabWidth\` is ${prettierOptions.tabWidth} while XO \`space\` is ${options.space}`,
    );
  }
...
const buildTSConfig = (options) =>
  (config) => {
    if (options.ts) {
      config.baseConfig.extends.push(
        require.resolve("eslint-config-xo-typescript"),
      );
...
const applyOverrides = (file, options) => {
  if (options.overrides && options.overrides.length > 0) {
    ...
    const { applicable, hash } = findApplicableOverrides(
      path.relative(options.cwd, file),
      overrides,
    );
    ...
    options = mergeWith(
      getEmptyXOConfig(),
      options,
      ...applicable.map((override) => normalizeOptions(override)),
      mergeFn,
    );
...
const gatherImportResolvers = (options) => {
  ...
  const resolverSettings = options.settings &&
    options.settings["import/resolver"];
  if (resolverSettings) {
  ...
  if (options.webpack) {
    ...
    const webpackConfigPath = findUpSync("webpack.config.js", {
      cwd: options.cwd,
    });
...
const parseOptions = async (options) => {
  options = normalizeOptions(options);
  const { options: foundOptions, prettierOptions } = await mergeWithFileConfig(
    options,
  );
  const { eslintConfigId, tsConfigHash, tsConfig, tsConfigPath } = foundOptions;
  const { filePath, warnIgnored, ...eslintOptions } = buildConfig(
    foundOptions,
    prettierOptions,
  );
  return {
...
const getOptionGroups = async (files, options) => {
  ...
  await Promise.all(
    Object.values(tsGroups).map(async (tsGroup) => {
      await fs.mkdir(path.dirname(tsGroup[0].tsConfigPath), {
        recursive: true,
      });
      await fs.writeFile(
        tsGroup[0].tsConfigPath,
        JSON.stringify({
          ...tsGroup[0].tsConfig,
