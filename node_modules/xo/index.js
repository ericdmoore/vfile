...
import { getIgnoredReport, mergeReports, processReport } from "./lib/report.js";
...
const lintText = async (string, options) => {
  const [[options_]] = Object.values(
    await getOptionGroups([options && options.filePath], options),
  );
  const { filePath, warnIgnored, eslintOptions, isQuiet } = options_;
  const { cwd, baseConfig: { ignorePatterns } } = eslintOptions;

  if (
    typeof filePath !== "string" && !isEqual(getIgnores({}), ignorePatterns)
  ) {
    throw new Error(
      "The `ignores` option requires the `filePath` option to be defined.",
    );
  }
