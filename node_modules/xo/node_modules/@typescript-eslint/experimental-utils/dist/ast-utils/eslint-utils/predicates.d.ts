...
declare type IsSpecificTokenFunction<SpecificToken extends TSESTree.Token> = (
  token: TSESTree.Token,
) => token is SpecificToken;
declare type IsNotSpecificTokenFunction<SpecificToken extends TSESTree.Token> =
  (token: TSESTree.Token) => token is Exclude<TSESTree.Token, SpecificToken>;
declare type PunctuatorTokenWithValue<Value extends string> =
  & TSESTree.PunctuatorToken
  & {
    value: Value;
  };
declare type IsPunctuatorTokenWithValueFunction<Value extends string> =
  IsSpecificTokenFunction<PunctuatorTokenWithValue<Value>>;
declare type IsNotPunctuatorTokenWithValueFunction<Value extends string> =
