...
var _ClassVisitor_classNode,
  _ClassVisitor_referencer,
  _ClassVisitor_emitDecoratorMetadata;
...
class ClassVisitor extends Visitor_1.Visitor {
  ...
  visitClass(node) {
    ...
    if (node.type === types_1.AST_NODE_TYPES.ClassDeclaration && node.id) {
      __classPrivateFieldGet(this, _ClassVisitor_referencer, "f")
        ...
        .defineIdentifier(
          node.id,
          new definition_1.ClassNameDefinition(node.id, node),
        );
    }
    (_a = node.decorators) === null || _a === void 0
      ? void 0
      : _a.forEach((d) =>
        __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").visit(d)
      );
    __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").scopeManager
      .nestClassScope(node);
    if (node.id) {
    ...
    __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").visit(
      node.superClass,
    );
    ...
    (_b = node.implements) === null || _b === void 0
      ? void 0
      : _b.forEach((imp) => this.visitType(imp));
  ...
  visitMethodFunction(node, methodNode) {
     ...
     * }
     */
    withMethodDecorators = withMethodDecorators ||
      (methodNode.kind !== "set" &&
    ...
    if (!withMethodDecorators && methodNode.kind === "set") {
      ...
      if (
        keyName !== null &&
        ((_a = __classPrivateFieldGet(this, _ClassVisitor_classNode, "f").body
                .body.find((node) =>
                  node !== methodNode &&
    ...
    for (const param of node.params) {
      this.visitPattern(param, (pattern, info) => {
        ...
        __classPrivateFieldGet(this, _ClassVisitor_referencer, "f")
          .referencingDefaultValue(pattern, info.assignments, null, true);
      }, { processRightHandNodes: true });
      this.visitFunctionParameterTypeAnnotation(param, withMethodDecorators);
      (_b = param.decorators) === null || _b === void 0
        ? void 0
        : _b.forEach((d) => this.visit(d));
    }
  ...
  visitProperty(node) {
    ...
    if (node.value) {
      if (node.type === types_1.AST_NODE_TYPES.PropertyDefinition) {
        __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").scopeManager
          .nestClassFieldInitializerScope(node.value);
      }
      __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").visit(
        node.value,
      );
      if (node.type === types_1.AST_NODE_TYPES.PropertyDefinition) {
        __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").close(
          node.value,
        );
      }
    }
  ...
  visitType(node) {
    ...
    TypeVisitor_1.TypeVisitor.visit(
      __classPrivateFieldGet(this, _ClassVisitor_referencer, "f"),
      node,
    );
  }
  visitMetadataType(node, withDecorators) {
    ...
    if (
      __classPrivateFieldGet(this, _ClassVisitor_classNode, "f").type ===
        types_1.AST_NODE_TYPES.ClassDeclaration &&
      ...
      if (
        node.typeAnnotation.typeName.type ===
          types_1.AST_NODE_TYPES.TSQualifiedName
      ) {
      ...
      if (withDecorators) {
        __classPrivateFieldGet(this, _ClassVisitor_referencer, "f")
          .currentScope().referenceDualValueType(identifier);
...
_ClassVisitor_classNode = new WeakMap(),
  _ClassVisitor_referencer = new WeakMap(),
  _ClassVisitor_emitDecoratorMetadata = new WeakMap();
/**
...
function getLiteralMethodKeyName(node) {
  if (node.computed && node.key.type === types_1.AST_NODE_TYPES.Literal) {
    ...
    !node.computed && node.key.type === types_1.AST_NODE_TYPES.Identifier
  ) {
