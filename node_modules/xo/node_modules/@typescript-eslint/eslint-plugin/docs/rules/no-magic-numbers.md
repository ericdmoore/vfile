...
See
[`eslint/no-magic-numbers` options](https://eslint.org/docs/rules/no-magic-numbers#options).
...
A boolean to specify if enums used in TypeScript are considered okay. `false` by
default.
...
A boolean to specify if numbers used in TypeScript numeric literal types are
considered okay. `false` by default.
...
`{ "ignoreReadonlyClassProperties": false }` option:
...
<sup>Taken with ❤️
[from ESLint core](https://github.com/eslint/eslint/blob/master/docs/rules/no-magic-numbers.md)</sup>
