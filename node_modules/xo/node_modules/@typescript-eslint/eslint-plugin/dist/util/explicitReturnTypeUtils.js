...
exports.isTypedFunctionExpression = exports
  .doesImmediatelyReturnFunctionExpression = exports.checkFunctionReturnType =
    exports.checkFunctionExpressionReturnType = void 0;
...
function isPropertyOfObjectWithType(property) {
  if (
    !property || property.type !== experimental_utils_1.AST_NODE_TYPES.Property
  ) {
    return false;
  }
...
function doesImmediatelyReturnFunctionExpression({ body }) {
  ...
  if (
    body.type === experimental_utils_1.AST_NODE_TYPES.BlockStatement &&
    body.body.length === 1
  ) {
    const [statement] = body.body;
    // Check if that statement is a return statement with an argument
    if (
      statement.type === experimental_utils_1.AST_NODE_TYPES.ReturnStatement &&
      !!statement.argument
    ) {
...
exports.doesImmediatelyReturnFunctionExpression =
...
function returnsConstAssertionDirectly(node) {
  ...
  if ((0, astUtils_1.isTypeAssertion)(body)) {
    ...
    if (
      ...
      if (
        typeName.type === experimental_utils_1.AST_NODE_TYPES.Identifier &&
        typeName.name === "const"
      ) {
        return true;
      }
...
function isTypedFunctionExpression(node, options) {
  const parent = (0, nullThrows_1.nullThrows)(
    node.parent,
    nullThrows_1.NullThrowsReasons.MissingParent,
  );
