...
class UnusedVarsVisitor extends Visitor_1.Visitor {
  ...
  constructor(context) {
    ...
    __classPrivateFieldSet(
      this,
      _UnusedVarsVisitor_scopeManager,
      (0, nullThrows_1.nullThrows)(
  ...
  getScope(currentNode) {
    ...
    const inner =
      currentNode.type !== experimental_utils_1.AST_NODE_TYPES.Program;
    let node = currentNode;
    while (node) {
      const scope = __classPrivateFieldGet(
        this,
        _UnusedVarsVisitor_scopeManager,
        "f",
      ).acquire(node, inner);
    ...
    return __classPrivateFieldGet(this, _UnusedVarsVisitor_scopeManager, "f")
      .scopes[0];
  }
  markVariableAsUsed(variableOrIdentifierOrName, parent) {
    ...
    while (currentScope) {
      const variable = currentScope.variables.find((scopeVar) =>
        scopeVar.name === name
      );
      if (variable) {
  ...
  ForInStatement(node) {
    ...
    let idOrVariable;
    if (
      node.left.type === experimental_utils_1.AST_NODE_TYPES.VariableDeclaration
    ) {
      const variable =
        __classPrivateFieldGet(this, _UnusedVarsVisitor_scopeManager, "f")
          .getDeclaredVariables(node.left)[0];
      if (!variable) {
        return;
      }
  ...
  Identifier(node) {
    const scope = this.getScope(node);
    if (
      scope.type === experimental_utils_1.TSESLint.Scope.ScopeType.function &&
      node.name === "this"
    ) {
...
function isMergableExported(variable) {
  ...
  for (const def of variable.defs) {
    ...
    if (
      def.type === experimental_utils_1.TSESLint.Scope.DefinitionType.Parameter
    ) {
      continue;
    }
    if (
      (MERGABLE_TYPES.has(def.node.type) &&
        ((_a = def.node.parent) === null || _a === void 0
            ? void 0
            : _a.type) ===
          experimental_utils_1.AST_NODE_TYPES.ExportNamedDeclaration) ||
      ((_b = def.node.parent) === null || _b === void 0 ? void 0 : _b.type) ===
...
function isExported(variable) {
  ...
  if (definition) {
    ...
    if (node.type === experimental_utils_1.AST_NODE_TYPES.VariableDeclarator) {
      ...
      definition.type ===
        experimental_utils_1.TSESLint.Scope.DefinitionType.Parameter
    ) {
      return false;
    }
...
function isUsedVariable(variable) {
  ...
  function getFunctionDefinitions(variable) {
    ...
    variable.defs.forEach((def) => {
      ...
      if (
        def.type ===
          experimental_utils_1.TSESLint.Scope.DefinitionType.FunctionName
      ) {
      ...
      if (
        def.type ===
          experimental_utils_1.TSESLint.Scope.DefinitionType.Variable &&
        (((_a = def.node.init) === null || _a === void 0 ? void 0 : _a.type) ===
            experimental_utils_1.AST_NODE_TYPES.FunctionExpression ||
          ((_b = def.node.init) === null || _b === void 0
              ? void 0
              : _b.type) ===
