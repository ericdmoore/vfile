...
function hasAssignmentBeforeNode(variable, node) {
  return (variable.references.some((ref) =>
    ref.isWrite() && ref.identifier.range[1] < node.range[1]
  ) ||
...
function isDefinitionWithAssignment(definition) {
  ...
  return (variableDeclarator.definite === true ||
      ...
      'LogicalExpression[operator = "??"] > TSNonNullExpression.left'(node) {
        if (
          node.expression.type ===
            experimental_utils_1.TSESTree.AST_NODE_TYPES.Identifier
        ) {
          const scope = context.getScope();
          const identifier = node.expression;
          const variable = experimental_utils_1.ASTUtils.findVariable(
            scope,
            identifier.name,
          );
              ...
              fix(fixer) {
                const exclamationMark = util.nullThrows(
                  sourceCode.getLastToken(
                    node,
                    experimental_utils_1.ASTUtils.isNonNullAssertionPunctuator,
                  ),
