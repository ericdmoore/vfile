...
function newKeywordTester(keyword) {
  return {
    test(node, sourceCode) {
      var _a;
      return ((_a = sourceCode.getFirstToken(node)) === null || _a === void 0
        ? void 0
        : _a.value) === keyword;
    },
  };
...
function isIIFEStatement(node) {
  if (node.type === experimental_utils_1.AST_NODE_TYPES.ExpressionStatement) {
    ...
    if (
      expression.type === experimental_utils_1.AST_NODE_TYPES.UnaryExpression
    ) {
      expression = skipChainExpression(expression.argument);
    }
    if (
      expression.type === experimental_utils_1.AST_NODE_TYPES.CallExpression
    ) {
      let node = expression.callee;
      while (
        node.type === experimental_utils_1.AST_NODE_TYPES.SequenceExpression
      ) {
...
function isCJSRequire(node) {
  if (node.type === experimental_utils_1.AST_NODE_TYPES.VariableDeclaration) {
    ...
    if (
      declaration === null || declaration === void 0 ? void 0 : declaration.init
    ) {
      let call = declaration === null || declaration === void 0
        ? void 0
        : declaration.init;
      while (
        call.type === experimental_utils_1.AST_NODE_TYPES.MemberExpression
      ) {
        call = call.object;
      }
      if (
        call.type === experimental_utils_1.AST_NODE_TYPES.CallExpression &&
...
function isBlockLikeStatement(node, sourceCode) {
  ...
  if (
    node.type === experimental_utils_1.AST_NODE_TYPES.DoWhileStatement &&
