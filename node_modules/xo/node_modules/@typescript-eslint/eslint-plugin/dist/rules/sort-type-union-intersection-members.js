...
function requiresParentheses(node) {
  return node.type === experimental_utils_1.AST_NODE_TYPES.TSFunctionType;
}
  ...
  create(context, [{ checkIntersections, checkUnions, groupOrder }]) {
    ...
    function checkSorting(node) {
      ...
      const sourceOrder = node.types.map((type) => {
        var _a;
        const group = (_a = groupOrder === null || groupOrder === void 0
                ? void 0
                : groupOrder.indexOf(getGroup(type))) !== null && _a !== void 0
          ? _a
          : -1;
        return {
          group: group === -1
        ...
        if (expectedOrder[i].node !== sourceOrder[i].node) {
          ...
          const fix = (fixer) => {
            const sorted = expectedOrder
              .map((t) => (requiresParentheses(t.node)
                ? `(${t.text})`
                : t.text)
              )
              .join(
