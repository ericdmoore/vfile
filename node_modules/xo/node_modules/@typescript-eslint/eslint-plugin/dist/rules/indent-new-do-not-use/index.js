...
exports.default = (0, util_1.createRule)({
  ...
  create(context, [userIndent, userOptions]) {
    ...
    const offsets = new OffsetStorage_1.OffsetStorage(
      tokenInfo,
      indentSize,
      indentType === "space" ? " " : "\t",
    );
    ...
    function isOuterIIFE(node) {
      ...
      /*
             ...
             */
      let statement = (_a = node.parent) === null || _a === void 0
        ? void 0
        : _a.parent;
      while (
        statement &&
    ...
    function addBlocklessNodeIndent(node) {
      if (node.type !== experimental_utils_1.AST_NODE_TYPES.BlockStatement) {
        ...
        while (
          (0, util_1.isOpeningParenToken)(
            sourceCode.getTokenBefore(firstBodyToken),
          ) &&
          (0, util_1.isClosingParenToken)(
    ...
    const baseOffsetListeners = {
      "ArrayExpression, ArrayPattern"(node) {
        ...
        const closingBracket = sourceCode.getTokenAfter(
          (_a = node.elements[node.elements.length - 1]) !== null &&
            _a !== void 0
            ? _a
            : openingBracket,
          util_1.isClosingBracketToken,
        );
        addElementListIndent(
          node.elements,
          openingBracket,
      ...
      ImportDeclaration(node) {
        ...
        const fromToken = sourceCode.getLastToken(
          node,
          (token) =>
            token.type === experimental_utils_1.AST_TOKEN_TYPES.Identifier &&
            token.value === "from",
        );
        const sourceToken = sourceCode.getLastToken(
          node,
          (token) => token.type === experimental_utils_1.AST_TOKEN_TYPES.String,
        );
      ...
      "MemberExpression, JSXMemberExpression, MetaProperty"(node) {
        const object =
          node.type === experimental_utils_1.AST_NODE_TYPES.MetaProperty
            ? node.meta
            : node.object;
        ...
        const firstNonObjectToken = sourceCode.getFirstTokenBetween(
          object,
          node.property,
          util_1.isNotClosingParenToken,
        );
        const secondNonObjectToken = sourceCode.getTokenAfter(
          firstNonObjectToken,
        );
        const objectParenCount =
          sourceCode.getTokensBetween(object, node.property, {
            filter: util_1.isClosingParenToken,
          }).length;
        ...
        const precedingTokens = sourceCode.ast.comments.reduce(
          (commentMap, comment) => {
            ...
            return commentMap.set(
              comment,
              (_a = commentMap.get(tokenBefore)) !== null && _a !== void 0
                ? _a
                : tokenBefore,
            );
          },
