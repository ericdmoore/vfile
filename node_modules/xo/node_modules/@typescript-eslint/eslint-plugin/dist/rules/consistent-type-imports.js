...
function isImportToken(token) {
  return token.type === experimental_utils_1.AST_TOKEN_TYPES.Keyword &&
    token.value === "import";
}
function isTypeToken(token) {
  return token.type === experimental_utils_1.AST_TOKEN_TYPES.Identifier &&
    token.value === "type";
}
exports.default = util.createRule({
  ...
  create(context, [option]) {
    var _a;
    const prefer = (_a = option.prefer) !== null && _a !== void 0
      ? _a
      : "type-imports";
        ...
        prefer === "type-imports"
          ? {
            ...
            ImportDeclaration(node) {
              ...
              const sourceImports =
                (_a = sourceImportsMap[source]) !== null && _a !== void 0
                  ? _a
                ...
                if (variable.references.length === 0) {
                  ...
                  const onlyHasTypeReferences = variable.references.every(
                    (ref) => {
                       ...
                       */
                      if (
                        ((_a = ref.identifier.parent) === null || _a === void 0
                            ? void 0
                            : _a.type) ===
                          experimental_utils_1.AST_NODE_TYPES.ExportSpecifier ||
                        ((_b = ref.identifier.parent) === null || _b === void 0
                            ? void 0
                            : _b.type) ===
                          experimental_utils_1.AST_NODE_TYPES
                      ...
                      if (ref.isValueReference) {
                        ...
                        while (parent) {
                          switch (parent.type) {
                            ...
                            case experimental_utils_1.AST_NODE_TYPES
                              .TSTypeQuery:
                              return true;
                            case experimental_utils_1.AST_NODE_TYPES
                              .TSQualifiedName:
                ...
                for (const report of sourceImports.reportValueImports) {
                  if (
                    ...
                    const importNames = (isTypeImport
                      ...
                      : report.typeSpecifiers).map((specifier) =>
                    ...
                    context.report(
                      Object.assign(
                        Object.assign({ node: report.node }, message),
                        {
                          *fix(fixer) {
                            if (isTypeImport) {
    ...
    function classifySpecifier(node) {
      ...
      const defaultSpecifier =
        node.specifiers[0].type ===
          ...
          ? node.specifiers[0]
          : null;
      const namespaceSpecifier = (_a = node.specifiers.find((specifier) =>
              specifier.type ===
      ...
      const namedSpecifiers = node.specifiers.filter((specifier) =>
        specifier.type === experimental_utils_1.AST_NODE_TYPES.ImportSpecifier
      );
      return {
        defaultSpecifier,
        namespaceSpecifier,
    ...
    function* fixToTypeImport(fixer, report, sourceImports) {
      ...
      if (typeNamedSpecifiers.length) {
        if (sourceImports.typeOnlyNamedImport) {
          ...
          yield fixer.insertTextBefore(
            node,
            `import type {${fixesNamedSpecifiers.typeNamedSpecifiersText}} from ${
      ...
      if (
        ...
        yield fixer.insertTextBefore(
          node,
          `import type ${sourceCode.getText(namespaceSpecifier)} from ${
            sourceCode.getText(node.source)
          };\n`,
        );
      }
        ...
        if (openingBraceToken) {
          ...
          if (node.specifiers.length > 1) {
            ...
            yield fixer.insertTextAfter(
              node,
              `\nimport type${specifiersText} from ${
                sourceCode.getText(node.source)
              };`,
            );
          }
    ...
    function* fixToValueImportInDecoMeta(fixer, report, sourceImports) {
      ...
      if (valueNamedSpecifiers.length) {
        if (sourceImports.valueOnlyNamedImport) {
          ...
          yield fixer.insertTextBefore(
            node,
            `import {${fixesNamedSpecifiers.typeNamedSpecifiersText}} from ${
              sourceCode.getText(node.source)
            };\n`,
          );
        }
    ...
    function* fixToValueImport(fixer, node) {
      ...
      const typeToken = util.nullThrows(
        sourceCode.getFirstTokenBetween(
          importToken,
          (_b = (_a = node.specifiers[0]) === null || _a === void 0
                ? void 0
                : _a.local) !== null && _b !== void 0
            ? _b
            : node.source,
          isTypeToken,
        ),
