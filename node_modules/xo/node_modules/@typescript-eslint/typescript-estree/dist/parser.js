...
exports.clearParseAndGenerateServicesCalls = exports.clearProgramCache = exports
  .parseWithNodeMaps = exports.parseAndGenerateServices = exports.parse =
    void 0;
...
const createIsolatedProgram_1 = require(
  "./create-program/createIsolatedProgram",
);
...
const semantic_or_syntactic_errors_1 = require(
...
function getProgramAndAST(
  ...
  return ((programInstances &&
    (0, useProvidedPrograms_1.useProvidedPrograms)(programInstances, extra)) ||
    (shouldProvideParserServices &&
      (0, createProjectProgram_1.createProjectProgram)(
        code,
        shouldCreateDefaultProgram,
        extra,
      )) ||
...
function getTsconfigPath(tsconfigPath, extra) {
  return (0, shared_1.getCanonicalFileName)(
    (0, shared_1.ensureAbsolutePath)(tsconfigPath, extra),
  );
}
/**
...
function prepareAndTransformProjects(projectsInput, ignoreListInput) {
  ...
  const nonGlobProjects = sanitizedProjects.filter((project) =>
    !(0, is_glob_1.default)(project)
  );
...
function applyParserOptionsToExtra(options) {
  ...
  if (!extra.programs) {
    ...
    const projectFolderIgnoreList =
      ((_a = options.projectFolderIgnoreList) !== null && _a !== void 0
        ? _a
        : ["**/node_modules/**"])
        .reduce((acc, folder) => {
...
function inferSingleRun(options) {
  ...
  if (
    options === null || options === void 0
      ? void 0
      : options.allowAutomaticSingleRunInference
  ) {
    if (
      ...
      process.argv[1].endsWith(
        (0, path_1.normalize)("node_modules/.bin/eslint"),
      )
    ) {
      extra.singleRun = true;
      return;
...
function parseWithNodeMapsInternal(code, options, shouldPreserveNodeMaps) {
   ...
   */
  const { estree, astMaps } = (0, ast_converter_1.astConverter)(
    ast,
    extra,
...
function parseAndGenerateServices(code, options) {
   ...
   */
  if (
    extra.singleRun && !extra.programs &&
    ((_a = extra.projects) === null || _a === void 0 ? void 0 : _a.length) > 0
  ) {
    extra.programs = {
      *[Symbol.iterator]() {
        for (const configFile of extra.projects) {
          ...
          if (existingProgram) {
            ...
            const newProgram =
              (0, useProvidedPrograms_1.createProgramFromConfigFile)(
                configFile,
              );
   ...
   */
  const shouldProvideParserServices = extra.programs != null ||
  ...
  if (program && extra.errorOnTypeScriptSyntacticAndSemanticIssues) {
    const error =
      (0, semantic_or_syntactic_errors_1.getFirstSemanticOrSyntacticError)(
        program,
        ast,
      );
    if (error) {
