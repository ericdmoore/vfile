...
exports.getProgramsForProjects = exports.createWatchProgram = exports
  .clearWatchCaches = void 0;
...
function updateCachedFileList(tsconfigPath, program, extra) {
  const fileList = extra.EXPERIMENTAL_useSourceOfProjectReferenceRedirect
    ? new Set(
      program.getSourceFiles().map((sf) =>
        (0, shared_1.getCanonicalFileName)(sf.fileName)
      ),
    )
    : new Set(
      program.getRootFileNames().map((f) =>
        (0, shared_1.getCanonicalFileName)(f)
      ),
    );
...
function getProgramsForProjects(code, filePathIn, extra) {
  ...
  for (const [tsconfigPath, existingWatch] of knownWatchProgramMap.entries()) {
    ...
    if (fileList.has(filePath)) {
      ...
      updatedProgram = updatedProgram !== null && updatedProgram !== void 0
        ? updatedProgram
...
const isRunningNoTimeoutFix = semver_1.default.satisfies(
  ts.version,
  ">=3.9.0-beta",
  {
...
function createWatchProgram(tsconfigPath, extra) {
  ...
  const watchCompilerHost = ts.createWatchCompilerHost(
    tsconfigPath,
    (0, shared_1.createDefaultCompilerOptionsFromExtra)(extra),
    ts.sys,
  ...
  if (extra.moduleResolver) {
    watchCompilerHost.resolveModuleNames =
      (0, shared_1.getModuleResolver)(extra.moduleResolver).resolveModuleNames;
  }
   ...
   */
  watchCompilerHost.useSourceOfProjectReferenceRedirect = () =>
...
function maybeInvalidateProgram(existingWatch, filePath, tsconfigPath) {
  ...
  const deletedFile = rootFilenames.find((file) =>
  ...
  const fileWatchCallbacks = fileWatchCallbackTrackingMap.get(
    (0, shared_1.getCanonicalFileName)(deletedFile),
  );
