...
exports.collectVariableUsage = exports.getDeclarationDomain = exports
  .getUsageDomain = exports.UsageDomain = exports.DeclarationDomain = void 0;
...
(function (DeclarationDomain) {
  ...
  DeclarationDomain = exports.DeclarationDomain ||
  ...
  finish(cb) {
    ...
    this._innerScope.end((variable, key, scope) => {
      if (
        ...
        !variable.exported &&
          (!this._ambient ||
            this._exports !== undefined && !this._exports.has(key.text))
      ) {
...
class UsageWalker {
  ...
  getUsage(sourceFile) {
    ...
    this._scope = new RootScope(
      sourceFile.isDeclarationFile && isModule &&
    ...
    const cb = (node) => {
      ...
      switch (node.kind) {
        ...
        case ts.SyntaxKind.TypeAliasDeclaration:
          ...
          return continueWithScope(
            node,
            new NonRootScope(this._scope, 4 /* Type */),
          );
        case ts.SyntaxKind.EnumDeclaration:
          ...
          return continueWithScope(
            ...
            this._scope.createOrReuseEnumScope(
              node.name.text,
              util_1.hasModifier(node.modifiers, ts.SyntaxKind.ExportKeyword),
            ),
          );
        case ts.SyntaxKind.ModuleDeclaration:
        ...
        case ts.SyntaxKind.EnumMember:
          this._scope.addVariable(
            util_1.getPropertyName(node.name),
            node.name,
            1, /* Function */
            true,
            4, /* Value */
  ...
  _handleModule(node, next) {
    if (node.flags & ts.NodeFlags.GlobalAugmentation) {
      return next(
        node,
        this._scope.createOrReuseNamespaceScope("-global", false, true, false),
      );
    }
  ...
  _handleVariableDeclaration(declarationList) {
    const blockScoped = util_1.isBlockScopedVariableDeclarationList(
      declarationList,
    );
    const exported =
...
function isNamespaceExported(node) {
  return node.parent.kind === ts.SyntaxKind.ModuleDeclaration ||
    util_1.hasModifier(node.modifiers, ts.SyntaxKind.ExportKeyword);
}
