...
exports.callExpressionAffectsControlFlow = exports.SignatureEffect = exports
  .getControlFlowEnd = exports.endsControlFlow = void 0;
...
function handleSwitchStatement(node, checker) {
  ...
  result.end &&
    (result.end = hasDefault ||
      checker !== undefined && util_1.hasExhaustiveCaseClauses(node, checker));
  return result;
}
  ...
  return {
    statements: tryResult.statements
      ...
      .concat(
        ...
        finallyResult === undefined ? [] : finallyResult.statements,
      ),
    ...
    switch (node.kind) {
      case ts.SyntaxKind.Identifier: {
        ...
        return isExplicitlyTypedSymbol(
          util_1.isSymbolFlagSet(symbol, ts.SymbolFlags.Alias)
      ...
      case ts.SyntaxKind.PropertyAccessExpression:
        if (
          !isExplicitlyTypedSymbol(checker.getSymbolAtLocation(node), checker)
        ) {
          return false;
        }
      // falls through
...
function isExplicitlyTypedSymbol(symbol, checker) {
  ...
  if (
    !util_1.isSymbolFlagSet(
      symbol,
      ts.SymbolFlags.Variable | ts.SymbolFlags.Property,
    )
  ) {
    return false;
  }
...
function declarationHasExplicitTypeAnnotation(node) {
  ...
  return (node_1.isVariableDeclaration(node) ||
    ...
    node_1.isPropertySignature(node)) &&
  ...
  do {
    ...
    if (node_1.isDecorator(node)) {
      ...
      if (
        ...
        node_1.isClassLikeDeclaration(node.parent.parent.parent)
      ) {
    ...
    util_1.isFunctionScopeBoundary(node) !== 1 /* Function */ ||
    node.kind === ts.SyntaxKind.ArrowFunction
  );
  return util_1.isFunctionWithBody(node) &&
      (util_1.isNodeFlagSet(node, ts.NodeFlags.JavaScriptFile)
        ? ((_a = ts.getJSDocThisTag(node)) === null || _a === void 0
          ? void 0
          : _a.typeExpression) !== undefined
        : node.parameters.length !== 0 &&
          util_1.isThisParameter(node.parameters[0]) &&
...
function callExpressionAffectsControlFlow(node, checker) {
  var _a, _b, _c;
  if (
    !node_1.isExpressionStatement(node.parent) ||
  ...
  const typeNode = ts.isJSDocSignature(signature.declaration)
    ? (_b = (_a = signature.declaration.type) === null || _a === void 0
            ? void 0
    ...
    ? _c
    : (util_1.isNodeFlagSet(signature.declaration, ts.NodeFlags.JavaScriptFile)
  ...
  if (
    ...
    typeNode.assertsModifier !== undefined
  ) {
    return 2 /* Asserts */;
  }
...
function matchLabel(current, label) {
  ...
  for (const statement of current.statements) {
    switch (statement.kind) {
      ...
      case ts.SyntaxKind.ContinueStatement:
        if (
          statement.label !== undefined && statement.label.text === labelText
        ) {
          result.end = false;
          continue;
        }
