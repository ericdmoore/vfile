...
exports.isValidIdentifier = exports.getLineBreakStyle = exports.getLineRanges =
  exports.forEachComment = exports.forEachTokenWithTrivia = exports
    .forEachToken = exports.isFunctionWithBody = exports.hasOwnThisReference =
      exports.isBlockScopeBoundary = exports.isFunctionScopeBoundary = exports
        .isTypeScopeBoundary = exports.isScopeBoundary = exports
          .ScopeBoundarySelector = exports.ScopeBoundary = exports
            .isInSingleStatementContext = exports
              .isBlockScopedDeclarationStatement = exports
                .isBlockScopedVariableDeclaration = exports
                  .isBlockScopedVariableDeclarationList = exports
                    .getVariableDeclarationKind = exports
                      .VariableDeclarationKind = exports
                        .forEachDeclaredVariable = exports
                          .forEachDestructuringIdentifier = exports
                            .getPropertyName = exports
                              .getWrappedNodeAtPosition = exports
                                .getAstNodeAtPosition = exports.commentText =
                                  exports.isPositionInComment = exports
                                    .getCommentAtPosition = exports
                                      .getTokenAtPosition = exports
                                        .getNextToken = exports
                                          .getPreviousToken = exports
                                            .getNextStatement = exports
                                              .getPreviousStatement = exports
                                                .isModifierFlagSet = exports
                                                  .isObjectFlagSet = exports
                                                    .isSymbolFlagSet = exports
                                                      .isTypeFlagSet = exports
                                                        .isNodeFlagSet = exports
                                                          .hasAccessModifier =
                                                            exports
                                                              .isParameterProperty =
                                                                exports
                                                                                                ...
                                                                                                exports
                                                                                                  .isTokenKind =
                                                                                                    exports
                                                                                                      .getChildOfKind =
                                                                                                        void 0;
exports.getBaseOfClassLikeExpression = exports.hasExhaustiveCaseClauses =
  exports.formatPseudoBigInt = exports.unwrapParentheses = exports
    .getSingleLateBoundPropertyNameOfPropertyName = exports
      .getLateBoundPropertyNamesOfPropertyName = exports
        .getLateBoundPropertyNames = exports.getPropertyNameOfWellKnownSymbol =
          exports.isWellKnownSymbolLiterally = exports
            .isBindableObjectDefinePropertyCall = exports
              .isReadonlyAssignmentDeclaration = exports.isInConstContext =
                exports.isConstAssertion = exports.getTsCheckDirective = exports
                  .getCheckJsDirective = exports.isAmbientModule = exports
                    .isCompilerOptionEnabled = exports
                      .isStrictCompilerOptionEnabled = exports.getIIFE = exports
                        .isAmbientModuleBlock = exports
                          .isStatementInAmbientContext = exports
                            .findImportLikeNodes = exports.findImports = exports
                              .ImportKind = exports.parseJsDocOfNode = exports
                                .getJsDoc = exports.canHaveJsDoc = exports
                                  .isReassignmentTarget = exports
                                    .getAccessKind = exports.AccessKind =
                                      exports.isExpressionValueUsed = exports
                                        .getDeclarationOfBindingElement =
                                          exports.hasSideEffects = exports
                                            .SideEffectOptions = exports
                                              .isSameLine = exports
                                                .isNumericPropertyName = exports
...
function isTypeNodeKind(kind) {
  return kind >= ts.SyntaxKind.FirstTypeNode &&
    kind <= ts.SyntaxKind.LastTypeNode;
}
...
function isKeywordKind(kind) {
  return kind >= ts.SyntaxKind.FirstKeyword &&
...
function isThisParameter(parameter) {
  return parameter.name.kind === ts.SyntaxKind.Identifier &&
    parameter.name.originalKeywordKind === ts.SyntaxKind.ThisKeyword;
}
...
function getTokenAtPositionWorker(node, pos, sourceFile, allowJsDoc) {
  ...
  while (true) {
    for (const child of node.getChildren(sourceFile)) {
      if (
        child.end > pos &&
        (allowJsDoc || child.kind !== ts.SyntaxKind.JSDocComment)
      ) {
...
function getAstNodeAtPosition(node, pos) {
  ...
  while (isNodeKind(node.kind)) {
    const nested = ts.forEachChild(
      node,
      (child) => child.pos <= pos && child.end > pos ? child : undefined,
    );
    if (nested === undefined) {
      break;
    }
...
(function (VariableDeclarationKind) {
  ...
  VariableDeclarationKind = exports.VariableDeclarationKind ||
...
(function (ScopeBoundarySelector) {
  ...
  ScopeBoundarySelector = exports.ScopeBoundarySelector ||
    (exports.ScopeBoundarySelector = {}),
);
function isScopeBoundary(node) {
  return isFunctionScopeBoundary(node) || isBlockScopeBoundary(node) ||
    isTypeScopeBoundary(node);
}
...
function getLineBreakStyle(sourceFile) {
  const lineStarts = sourceFile.getLineStarts();
  return lineStarts.length === 1 || lineStarts[1] < 2 ||
...
function isValidPropertyName(text, languageVersion = ts.ScriptTarget.Latest) {
  ...
  return scan.getTextPos() === text.length &&
    scan.getToken() === ts.SyntaxKind.NumericLiteral &&
    scan.getTokenValue() === text; // ensure stringified number equals literal
}
...
function isValidNumericLiteral(text, languageVersion = ts.ScriptTarget.Latest) {
  const scan = scanToken(text, languageVersion);
  return scan.getToken() === ts.SyntaxKind.NumericLiteral &&
    scan.getTextPos() === text.length && scan.getTokenPos() === 0;
}
...
function isValidJsxIdentifier(text, languageVersion = ts.ScriptTarget.Latest) {
  ...
  for (let i = charSize(ch); i < text.length; i += charSize(ch)) {
    ...
    if (!ts.isIdentifierPart(ch, languageVersion) && ch !== 45 /* minus */) {
      if (
        !seenNamespaceSeparator && ch === 58 /* colon */ &&
...
(function (SideEffectOptions) {
  ...
  SideEffectOptions = exports.SideEffectOptions ||
    ...
    switch (node.kind) {
          ...
          if (!hasModifier(child.modifiers, ts.SyntaxKind.DeclareKeyword)) {
            if (
              ((_a = child.name) === null || _a === void 0
                ? void 0
                : _a.kind) === ts.SyntaxKind.ComputedPropertyName
            ) {
      ...
      case ts.SyntaxKind.ObjectLiteralExpression:
        for (const child of node.properties) {
          if (
            ((_b = child.name) === null || _b === void 0 ? void 0 : _b.kind) ===
...
function parseJsDocWorker(
  ...
  const start = ts
    ...
    ](sourceFile.text, node.pos, // return object to make `0` a truthy value
    (pos, _end, kind) =>
...
class ImportFinder {
  ...
  _findNestedImports() {
    const isJavaScriptFile = this._ignoreFileName ||
      (this._sourceFile.flags & ts.NodeFlags.JavaScriptFile) !== 0;
    let re;
    let includeJsDoc;
    if ((this._options & 56 /* AllNestedImports */) === 16 /* Require */) {
      ...
      re = /\bimport\s*[</(]/g;
      includeJsDoc = isJavaScriptFile &&
        (this._options & 32 /* ImportType */) !== 0;
    }
    for (
      let match = re.exec(this._sourceFile.text);
      match !== null;
      match = re.exec(this._sourceFile.text)
    ) {
...
function getIIFE(func) {
  ...
  return node_1.isCallExpression(node) && func.end <= node.expression.end
    ? node
    : undefined;
}
...
function isCompilerOptionEnabled(options, option) {
  switch (option) {
    ...
    case "skipDefaultLibCheck":
      return options.skipDefaultLibCheck ||
        isCompilerOptionEnabled(options, "skipLibCheck");
    case "suppressImplicitAnyIndexErrors":
      return options.suppressImplicitAnyIndexErrors === true &&
    ...
    case "noUncheckedIndexedAccess":
      return options.noUncheckedIndexedAccess === true &&
...
function getLateBoundPropertyNames(node, checker) {
  ...
  if (isTsBefore43 && isWellKnownSymbolLiterally(node)) {
    ...
    for (
      const key of type_1.unionTypeParts(
...
function getPrimitiveLiteralFromType(t) {
  ...
  if (exports.isTypeFlagSet(t, ts.TypeFlags.NumberLiteral)) {
    return `${
      exports.isTypeFlagSet(t, ts.TypeFlags.EnumLiteral) ? "enum:" : ""
    }${t.value}`;
  }
...
function getBaseOfClassLikeExpression(node) {
  var _a;
  if (
    ((_a = node.heritageClauses) === null || _a === void 0
      ? void 0
      : _a[0].token) === ts.SyntaxKind.ExtendsKeyword
  ) {
