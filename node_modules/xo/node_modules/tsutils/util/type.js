...
exports.getBaseClassMemberOfClassElement = exports
  .getIteratorYieldResultFromIteratorResult = exports
    .getInstanceTypeOfClassLikeDeclaration = exports
      .getConstructorTypeOfClassLikeDeclaration = exports
        .getSymbolOfClassLikeDeclaration = exports.getPropertyNameFromType =
          exports.symbolHasReadonlyDeclaration = exports
            .isPropertyReadonlyInType = exports
              .getWellKnownSymbolPropertyOfType = exports.getPropertyOfType =
                exports.isBooleanLiteralType = exports.isFalsyType = exports
                  .isThenableType = exports.someTypePart = exports
                    .intersectionTypeParts = exports.unionTypeParts = exports
                      .getCallSignaturesOfType = exports
                        .isTypeAssignableToString = exports
                          .isTypeAssignableToNumber = exports
                            .isOptionalChainingUndefinedMarkerType = exports
                              .removeOptionalChainingUndefinedMarkerType =
                                exports.removeOptionalityFromType = exports
                                  .isEmptyObjectType = void 0;
...
function removeOptionalChainingUndefinedMarkerType(checker, type) {
  if (!type_1.isUnionType(type)) {
    return isOptionalChainingUndefinedMarkerType(checker, type)
...
exports.removeOptionalChainingUndefinedMarkerType =
  removeOptionalChainingUndefinedMarkerType;
function isOptionalChainingUndefinedMarkerType(checker, t) {
  return util_1.isTypeFlagSet(t, ts.TypeFlags.Undefined) &&
    checker.getNullableType(t.getNonNullableType(), ts.TypeFlags.Undefined) !==
      t;
}
exports.isOptionalChainingUndefinedMarkerType =
...
function isTypeAssignableTo(checker, type, flags) {
  ...
  return (function check(t) {
    if (
      type_1.isTypeParameter(t) && t.symbol !== undefined &&
...
function getPropertyNameOfWellKnownSymbol(
  ...
  const knownSymbolType = knownSymbol &&
    checker.getTypeOfSymbolAtLocation(
      knownSymbol,
...
function isPropertyReadonlyInType(type, name, checker) {
  ...
  for (const t of unionTypeParts(type)) {
    if (getPropertyOfType(t, name) === undefined) {
      ...
      const index = (util_1.isNumericPropertyName(name)
      ...
      seenReadonlySignature || isReadonlyPropertyIntersection(t, name, checker)
    ) {
      return true;
    } else {
      seenProperty = true;
...
function isReadonlyPropertyFromMappedType(type, name, checker) {
  if (
    !type_1.isObjectType(type) ||
    !util_1.isObjectFlagSet(type, ts.ObjectFlags.Mapped)
  ) {
    return;
  }
...
function symbolHasReadonlyDeclaration(symbol, checker) {
  return (symbol.flags & ts.SymbolFlags.Accessor) ===
      ts.SymbolFlags.GetAccessor ||
    symbol.declarations !== undefined &&
      symbol.declarations.some((node) =>
        util_1.isModifierFlagSet(node, ts.ModifierFlags.Readonly) ||
        node_1.isVariableDeclaration(node) &&
          util_1.isNodeFlagSet(node.parent, ts.NodeFlags.Const) ||
        node_1.isCallExpression(node) &&
          util_1.isReadonlyAssignmentDeclaration(node, checker) ||
        node_1.isEnumMember(node) ||
        (node_1.isPropertyAssignment(node) ||
            node_1.isShorthandPropertyAssignment(node)) &&
          util_1.isInConstContext(node.parent)
      );
}
...
function isGlobalDeclaration(node) {
  return util_1.isNodeFlagSet(node.parent, ts.NodeFlags.GlobalAugmentation) ||
    node_1.isSourceFile(node.parent) && !ts.isExternalModule(node.parent);
}
function getSymbolOfClassLikeDeclaration(node, checker) {
  ...
  return checker.getSymbolAtLocation(
    (_a = node.name) !== null && _a !== void 0
      ? _a
      : util_1.getChildOfKind(node, ts.SyntaxKind.ClassKeyword),
  );
}
...
function getConstructorTypeOfClassLikeDeclaration(node, checker) {
  return node.kind === ts.SyntaxKind.ClassExpression
    ...
    : checker.getTypeOfSymbolAtLocation(
      getSymbolOfClassLikeDeclaration(node, checker),
      node,
    );
}
exports.getConstructorTypeOfClassLikeDeclaration =
...
exports.getInstanceTypeOfClassLikeDeclaration =
        ...
        return done !== undefined &&
          isBooleanLiteralType(
            removeOptionalityFromType(
              checker,
              checker.getTypeOfSymbolAtLocation(done, node),
            ),
            false,
          );
      }) || type;
}
exports.getIteratorYieldResultFromIteratorResult =
...
function getBaseClassMemberOfClassElement(node, checker) {
  ...
  const baseType = checker.getTypeAtLocation(
    util_1.hasModifier(node.modifiers, ts.SyntaxKind.StaticKeyword)
      ? base.expression
      : base,
  );
