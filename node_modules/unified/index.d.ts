...
type UsePlugin<
  ...
  Input = void,
  Output = void,
> = Output extends Node ? Input extends string ? // If `Input` is `string` and `Output` is `Node`, then this plugin
...
Processor<
  ...
  CompileResult
>
: Input extends Node ? // If `Input` is `Node` and `Output` is `Node`, then this plugin defines a
...
never
  : Input extends Node ? // If `Input` is `Node` and `Output` is not a `Node`, then this plugin
...
export type Plugin<
  ...
  Output = Input,
> = (
  this: Input extends Node ? Output extends Node ? // This is a transform, so define `Input` as the current tree.
  Processor<void, Input>
  : // Compiler.
  Processor<void, Input, Input, Output>
    : Output extends Node ? // Parser.
    Processor<Output, Output>
    : // No clue.
    Processor,
  ...settings: PluginParameters
) => // If both `Input` and `Output` are `Node`, expect an optional `Transformer`.
Input extends Node ? Output extends Node ? Transformer<Input, Output> | void
: void
  : void;
