...
(function () {
  ...
  function k(Z) {
    ...
    function ab(ah) {
      ...
      if (ag) return ag;
        ...
        if ("0" <= af && af <= "7") return parseInt(ah.substring(1), 8);
        else {
          if (af === "u" || af === "x") return parseInt(ah.substring(2), 16);
          else return ah.charCodeAt(1);
        }
      }
    }
    function T(af) {
      if (af < 32) return (af < 16 ? "\\x0" : "\\x") + af.toString(16);
      var ag = String.fromCharCode(af);
      if (ag === "\\" || ag === "-" || ag === "[" || ag === "]") ag = "\\" + ag;
      return ag;
    }
    function X(am) {
      var aq = am.substring(1, am.length - 1).match(
        new RegExp(
      ...
      for (var ar = 0; ar < af.length; ++ar) {
        var at = af[ar];
        if (at[0] <= ap[1] + 1) ap[1] = Math.max(ap[1], at[1]);
        else ai.push(ap = at);
      }
      var an = ["["];
      if (ao) an.push("^");
    ...
    function W(al) {
      ...
      for (var ak = 1; ak < an.length; ++ak) if (-1 === an[ak]) an[ak] = ++ad;
      for (var ak = 0, am = 0; ak < ah; ++ak) {
        var ag = aj[ak];
        if (ag === "(") {
          ++am;
          if (an[am] === undefined) aj[ak] = "(?:";
        } else {
          if ("\\" === ag.charAt(0)) {
            var af = +ag.substring(1);
            if (af && af <= am) aj[ak] = "\\" + an[am];
          }
        }
      }
      for (var ak = 0, am = 0; ak < ah; ++ak) {
        if ("^" === aj[ak] && "^" !== aj[ak + 1]) aj[ak] = "";
      }
      if (al.ignoreCase && S) {
        for (var ak = 0; ak < ah; ++ak) {
          var ag = aj[ak];
          var ai = ag.charAt(0);
          if (ag.length >= 2 && ai === "[") aj[ak] = X(ag);
          else {
            if (ai !== "\\") {
    ...
    for (var V = 0, U = Z.length; V < U; ++V) {
      var ae = Z[V];
      if (ae.global || ae.multiline) throw new Error("" + ae);
      aa.push("(?:" + W(ae) + ")");
    }
  ...
  function a(V) {
    ...
    var W = 0;
    var S;
    if (V.currentStyle) S = V.currentStyle.whiteSpace;
    else {
      if (window.getComputedStyle) {
        S = document.defaultView.getComputedStyle(V, null).getPropertyValue(
          "white-space",
        );
      }
    }
    ...
    function aa(ab) {
      switch (ab.nodeType) {
        case 1:
          if (U.test(ab.className)) return;
          for (var ae = ab.firstChild; ae; ae = ae.nextSibling) aa(ae);
          var ad = ab.nodeName;
          if ("BR" === ad || "LI" === ad) {
            X[W] = "\n";
            Z[W << 1] = T++;
  ...
  function g(U, T) {
    ...
    (function () {
      ...
      for (var ab = 0, Z = ad.length; ab < Z; ++ab) {
        var Y = ad[ab];
        var ac = Y[3];
        if (ac) for (var ae = ac.length; --ae >= 0;) S[ac.charAt(ae)] = Y;
        var af = Y[1];
        var aa = "" + af;
  ...
  function i(T) {
    ...
    if (T.verbatimStrings) S.push([C, /^@\"(?:[^\"]|\"\")*(?:\"|$)/, null]);
  ...
  function Q(V, ag) {
    ...
    if (V.currentStyle) S = V.currentStyle.whiteSpace;
      ...
      if (window.getComputedStyle) {
        S = ac.defaultView.getComputedStyle(V, null).getPropertyValue(
          "white-space",
        );
      }
    }
    ...
    while (V.firstChild) af.appendChild(V.firstChild);
    var W = [af];
    function ae(al) {
      switch (al.nodeType) {
        case 1:
          if (U.test(al.className)) break;
          if ("BR" === al.nodeName) {
            ad(al);
            if (al.parentNode) al.parentNode.removeChild(al);
          } else for (var an = al.firstChild; an; an = an.nextSibling) ae(an);
          break;
        case 3:
        case 4:
          if (Z) {
            ...
            if (aj) {
              ...
              ad(al);
              if (!ai) al.parentNode.removeChild(al);
            }
          }
          break;
      }
    ...
    function ad(ak) {
      ...
      for (var aj; (aj = ah.parentNode) && aj.nodeType === 1;) ah = aj;
      W.push(ah);
    }
    for (var Y = 0; Y < W.length; ++Y) ae(W[Y]);
    if (ag === (ag | 0)) W[0].setAttribute("value", ag);
    ...
    for (var Y = 0, T = W.length; Y < T; ++Y) {
      ...
      if (!af.firstChild) af.appendChild(ac.createTextNode("\xA0"));
      aa.appendChild(af);
    }
    V.appendChild(aa);
  }
  ...
  function b(ad) {
    ...
    function U() {
      ...
      for (; X < T.length && W.now() < ag; X++) {
        ...
        if (ai.indexOf("prettyprint") >= 0) {
          ...
          for (var ak = aj.parentNode; ak; ak = ak.parentNode) {
            if (
              (ak.tagName === "pre" || ak.tagName === "code" ||
                ak.tagName === "xmp") &&
              ak.className && ak.className.indexOf("prettyprint") >= 0
            ) {
              al = true;
              break;
            }
      ...
      if (X < T.length) setTimeout(U, 250);
      else if (ad) ad();
    }
    U();
  }
...
PR.registerLangHandler(
  PR.createSimpleLexer([[PR.PR_PLAIN, /^[ \t\r\n\f]+/, null, " \t\r\n\f"]], [
    [PR.PR_STRING, /^\"(?:[^\n\r\f\\\"]|\\(?:\r\n?|\n|\f)|\\[\s\S])*\"/, null],
    [PR.PR_STRING, /^\'(?:[^\n\r\f\\\']|\\(?:\r\n?|\n|\f)|\\[\s\S])*\'/, null],
    ["lang-css-str", /^url\(([^\)\"\']*)\)/i],
    [
