...
const compatError = Error(
  `requires Node.js ${require("../package.json").engines.node}`,
);
let readFile = () => {
  throw compatError;
};
...
const cjsWrapperLength = isOlderNode10
...
module.exports = class V8ToIstanbul {
  constructor(scriptPath, wrapperLength, sources, excludePath) {
    ...
    assert(
      !isNode8,
      "This module does not support node 8 or lower, please upgrade to node 10",
    );
    this.path = parsePath(scriptPath);
    this.wrapperLength = wrapperLength === undefined
      ? cjsWrapperLength
      : wrapperLength;
  ...
  async load() {
    ...
    this.rawSourceMap = this.sources.sourceMap ||
      ...
      convertSourceMap.fromSource(rawSource) ||
      convertSourceMap.fromMapFileSource(rawSource, dirname(this.path));
    ...
    if (this.rawSourceMap) {
      if (this.rawSourceMap.sourcemap.sources.length > 1) {
        ...
        if (!this.sourceMap.sourcesContent) {
          this.sourceMap.sourcesContent = await this
            .sourcesContentFromSources();
        }
        ...
        const candidatePath = this.rawSourceMap.sourcemap.sources.length >= 1
          ? this.rawSourceMap.sourcemap.sources[0]
          : this.rawSourceMap.sourcemap.file;
        this.path = this._resolveSource(
          this.rawSourceMap,
          candidatePath || this.path,
        );
        this.sourceMap = await new SourceMapConsumer(
          this.rawSourceMap.sourcemap,
        );
        ...
        if (
          this.sources.sourceMap && this.sources.sourceMap.sourcemap &&
          this.sources.sourceMap.sourcemap.sourcesContent &&
          this.sources.sourceMap.sourcemap.sourcesContent.length === 1
        ) {
          ...
          originalRawSource =
            this.sources.sourceMap.sourcemap.sourcesContent[0];
          ...
          this.sourceMap.sourcesContent && this.sourceMap.sourcesContent[0]
        ) {
  ...
  _resolveSource(rawSourceMap, sourcePath) {
    ...
    const sourceRoot = rawSourceMap.sourcemap.sourceRoot
      ? rawSourceMap.sourcemap.sourceRoot.replace("file://", "")
      : "";
  ...
  applyCoverage(blocks) {
    blocks.forEach((block) => {
      block.ranges.forEach((range, i) => {
        const { startCol, endCol, path, covSource } = this
        ...
        lines.forEach((line) => {
          ...
          if (
            startCol <= line.startCol && endCol >= line.endCol && !line.ignore
          ) {
  ...
  _maybeRemapStartColEndCol(range) {
    ...
    let endCol = Math.min(
      covSource.eof,
      range.endOffset - covSource.wrapperLength,
    );
    let path = this.path;
    ...
    if (this.sourceMap) {
      startCol = Math.max(
        0,
        range.startOffset - this.sourceTranspiled.wrapperLength,
      );
      endCol = Math.min(
        this.sourceTranspiled.eof,
        range.endOffset - this.sourceTranspiled.wrapperLength,
      );

      const { startLine, relStartCol, endLine, relEndCol, source } = this
        .sourceTranspiled.offsetToOriginalRelative(
          this.sourceMap,
          startCol,
          endCol,
        );

      const matchingSource = this.covSources.find((covSource) =>
        covSource.path === source
      );
      covSource = matchingSource
        ? matchingSource.source
        : this.covSources[0].source;
  ...
  toIstanbul() {
    return this.covSources.reduce(
      (istanbulOuter, { source, path }) =>
        Object.assign(istanbulOuter, this.getInnerIstanbul(source, path)),
      {},
    );
  }
...
function parsePath(scriptPath) {
  return scriptPath.startsWith("file://")
