...
const { GREATEST_LOWER_BOUND, LEAST_UPPER_BOUND } =
...
module.exports = class CovSource {
  ...
  _parseIgnore(lineStr) {
    const testIgnoreNextLines = lineStr.match(
      /^\W*\/\* c8 ignore next (?<count>[0-9]+) *\*\/\W*$/,
    );
    ...
    const testIgnoreStartStop = lineStr.match(
      /\/\* c8 ignore (?<mode>start|stop) *\*\//,
    );
  ...
  relativeToOffset(line, relCol) {
    ...
    return Math.min(
      this.lines[line - 1].startCol + relCol,
      this.lines[line - 1].endCol,
    );
  }
};
