...
type FixPathSquareBrackets<Path extends string> = Path extends
  `${infer Head}[${infer Middle}]${infer Tail}`
...
type PropertyOf<BaseType, Key extends string> = BaseType extends
  ...
  : Key extends keyof BaseType ? BaseType[Key]
  : BaseType extends {
    [n: number]: infer Item;
    length: number; // Note: This is needed to avoid being too lax with records types using number keys like `{0: string; 1: boolean}`.
  } ? (
    ConsistsOnlyOf<Key, StringDigit> extends true ? Item
      : unknown
  )
  : Key extends keyof WithStringKeys<BaseType> ? WithStringKeys<BaseType>[Key]
  : unknown;
...
```
*/
export type Get<BaseType, Path extends string> = GetWithPath<
  BaseType,
  ToPath<Path>
>;
