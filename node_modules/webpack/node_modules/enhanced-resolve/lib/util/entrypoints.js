...
function createFieldProcessor(treeRoot, assertRequest, assertTarget) {
  return function fieldProcessor(request, conditionNames) {
    ...
    const remainingRequest = remainRequestIndex === request.length + 1
      ? undefined
...
function assertExportTarget(exp, expectFolder) {
  if (
    ...
    throw new Error(
      `Export should be relative path and start with "./", got ${
        JSON.stringify(
          exp,
        )
      }.`,
  ...
  if (isFolder !== expectFolder) {
    throw new Error(
      expectFolder
        ...
        : `Expecting file to file mapping. ${
          JSON.stringify(
            exp,
          )
        } should not end with "/"`,
...
function findMatch(request, treeRoot) {
  ...
  const remainingRequest = lastNonSlashIndex > 0
    ? request.slice(lastNonSlashIndex)
    : request;
      ...
      if (i !== last) {
        ...
        if (isConditionalMapping(innerMapping)) {
          const conditionalMapping =
            /** @type {ConditionalMapping} */ (innerMapping);
      ...
      if (i === 0) {
        while (i < keys.length) {
          ...
          if (charCode === dotCode || charCode === slashCode) {
            throw new Error(
              `Exports field key should be relative path and start with "." (key: ${
                JSON.stringify(
                  key,
                )
              })`,
...
function buildImportsFieldPathTree(field) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (key.length === 1) {
      throw new Error(
        `Imports field key should have at least 2 characters (key: ${
          JSON.stringify(
            key,
          )
        })`,
    ...
    if (key.charCodeAt(1) === slashCode) {
      throw new Error(
        `Imports field key should not start with "#/" (key: ${
          JSON.stringify(
            key,
          )
        })`,
