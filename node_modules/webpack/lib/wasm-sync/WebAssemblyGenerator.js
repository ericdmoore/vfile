 ...
 * @returns {ArrayBufferTransform} transform
 */
const rewriteImportedGlobals = (state) =>
  (bin) => {
 ...
 * @returns {ArrayBufferTransform} transform
 */
const rewriteImports = ({ ast, usedDependencyMap }) =>
  (bin) => {
    return editWithAST(ast, bin, {
      ModuleImport(path) {
...
const getUsedDependencyMap = (moduleGraph, module, mangle) => {
  ...
  const map = new Map();
  for (
    const usedDep of WebAssemblyUtils.getUsedDependencies(
      moduleGraph,
      module,
      mangle,
    )
