...
const getArguments = (schema = webpackSchema) => {
  ...
  const addResetFlag = (path) => {
    ...
    const description = getResetDescription(path) ||
      `Clear all items provided in '${schemaPath}' configuration. ${
        getDescription(
          path,
        )
      }`;
      ...
      if (multiple) {
        throw new Error(
          `Conflicting schema for ${
            path[0].path
          } with ${argConfig.type} type (array type must be before single item type)`,
        );
      }
      return 0;
    }
