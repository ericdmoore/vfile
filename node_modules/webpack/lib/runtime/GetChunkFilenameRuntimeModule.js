  ...
  generate() {
    ...
    if (allChunks) {
      ...
      if (includeEntries) {
        ...
        for (
          const c of chunkGraph.getChunkEntryDependentChunksIterable(
            chunk,
          )
        ) {
          addChunk(c);
    ...
    const addStaticUrl = (c, chunkFilename) => {
      ...
      const unquotedStringifyWithLength = (value) =>
        (length) => unquotedStringify(`${value}`.slice(0, length));
      const chunkFilenameValue = typeof chunkFilename === "function"
        ? JSON.stringify(
          chunkFilename({
            chunk: c,
    ...
    const createMap = (fn) => {
      ...
      if (entries === 1) {
        return useId
          ? `(chunkId === ${JSON.stringify(lastKey)} ? ${
            JSON.stringify(
              obj[lastKey],
            )
          } : chunkId)`
    ...
    return Template.asString([
      `// This function allow to reference ${
        includedChunksMessages.join(
          " and ",
        )
      }`,
      `${global} = ${
