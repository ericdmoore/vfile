...
const accessWithInit = (accessor, existingLength, initLast = false) => {
  ...
  for (; i < initUntil; i++) {
    ...
    current = `(${current}${propertyAccess([prop])} = ${base}${
      propertyAccess(
        propsSoFar,
      )
    } || {})`;
...
class AssignLibraryPlugin extends AbstractLibraryPlugin {
  ...
  render(source, { chunk }, { options, compilation }) {
    ...
    if (this.declare) {
      ...
      if (!isNameValid(base)) {
        throw new Error(
          `Library name base (${base}) must be a valid identifier when using a var declaring library type. Either use a valid identifier (e. g. ${
            Template.toIdentifier(
              base,
            )
          }) or use a different library type (e. g. 'type: "global"', which assign a property on the global scope instead of declaring a variable). ${AbstractLibraryPlugin.COMMON_LIBRARY_NAME_MESSAGE}`,
        );
      }
  ...
  embedInRuntimeBailout(
    ...
    const topLevelDeclarations = (data && data.get("topLevelDeclarations")) ||
