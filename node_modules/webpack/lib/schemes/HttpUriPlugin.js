  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap(
      ...
      (compilation, { normalModuleFactory }) => {
        ...
        const lockfileLocation = this._lockfileLocation ||
          join(
            intermediateFs,
            compiler.context,
            compiler.name
        ...
        const cacheLocation = this._cacheLocation !== undefined
          ? this._cacheLocation
        ...
        const _getCacheKey = (url) => {
          ...
          return `${folder.slice(-50)}/${
            `${basename}${query ? `_${query}` : ""}`.slice(0, 150)
          }_${digest}${ext}`;
        };
              ...
              (res) => {
                ...
                stream.on("end", () => {
                  ...
                  if (res.statusCode !== 200) {
                    ...
                    return callback(
                      new Error(
                        `${url} request status code = ${res.statusCode}\n${
                  ...
                  fs.readFile(filePath, (err, result) => {
                    ...
                    if (!verifyIntegrity(content, entry.integrity)) {
                      ...
                      if (isEolChanged) {
                        if (!warnedAboutEol) {
                          const explainer =
                            `Incorrect end of line sequence was detected in the lockfile cache.
...
This will avoid that the end of line sequence is changed by git on Windows.`;
                      ...
                      if (frozen) {
                        return callback(
                          new Error(
                            `${entry.resolved} integrity mismatch, expected content with integrity ${entry.integrity} but got ${
                              computeIntegrity(content)
                            }.
Lockfile corrupted (${
                              isEolChanged
