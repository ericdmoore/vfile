...
class RuntimeTemplate {
  ...
  throwMissingModuleErrorFunction({ request }) {
    return `function webpackMissingModule() { ${
      this.throwMissingModuleErrorBlock(
        { request },
      )
    } }`;
  }
  ...
  missingModulePromise({ request }) {
    return `Promise.resolve().then(${
      this.throwMissingModuleErrorFunction({
        request,
      })
    })`;
  }
  ...
  weakError({ module, chunkGraph, request, idExpr, type }) {
    ...
    const errorMessage = moduleId === null
  ...
  moduleNamespace({
    ...
    if (chunkGraph.getModuleId(module) === null) {
      ...
      throw new Error(
        `RuntimeTemplate.moduleNamespace(): ${
          noModuleIdErrorMessage(
            module,
            chunkGraph,
          )
  ...
  moduleNamespacePromise({
    ...
    if (weak) {
      ...
      header += `if(!${RuntimeGlobals.moduleFactories}[${idExpr}]) { ${
        this.weakError({
          module,
          chunkGraph,
          request,
    ...
    switch (exportsType) {
      case "namespace":
        if (header) {
          ...
          runtimeRequirements.add(RuntimeGlobals.require);
          appending =
            `.then(__webpack_require__.bind(__webpack_require__, ${comment}${idExpr}))`;
        }
        break;
      case "dynamic":
        fakeType |= 4;
        ...
        if (chunkGraph.moduleGraph.isAsync(module)) {
          ...
          if (header) {
            const returnExpression =
              `${RuntimeGlobals.createFakeNamespaceObject}(${moduleIdExpr}, ${fakeType})`;
            appending = `.then(${
              this.basicFunction(
                "",
            ...
            appending =
              `.then(${RuntimeGlobals.createFakeNamespaceObject}.bind(__webpack_require__, ${comment}${idExpr}, ${fakeType}))`;
          }
        }
        break;
    }
  ...
  runtimeConditionExpression({
    ...
    forEachRuntime(
      subtractRuntime(runtime, runtimeCondition),
      (runtime) =>
  ...
  exportFromImport({
    ...
    if (defaultInterop) {
      if (exportName.length > 0 && exportName[0] === "default") {
        ...
        }(${importVar}_namespace_cache || (${importVar}_namespace_cache = ${RuntimeGlobals.createFakeNamespaceObject}(${importVar}${
          exportsType === "default-only" ? "" : ", 2"
        })))`;
      }
    }
  ...
  blockPromise({ block, message, chunkGraph, runtimeRequirements }) {
    ...
    if (chunks.length === 1) {
      ...
      return `Promise.all(${comment.trim()}[${
        chunks
          .map(requireChunkId)
          .join(", ")
      }])`;
