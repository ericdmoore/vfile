...
const getFinalBinding = (
  ...
  switch (info.type) {
    case "concatenated": {
      ...
      const reexport = exportInfo.findTarget(
        moduleGraph,
        (module) => moduleToInfoMap.has(module),
      );
      if (reexport === false) {
        throw new Error(
          `Target module of reexport from '${
...
class ConcatenatedModule extends Module {
  ...
  build(options, compilation, resolver, fs, callback) {
    ...
    for (const m of this._modules) {
      ...
      for (
        const d of m.dependencies.filter(
          (dep) =>
  ...
  _createConcatenationList(rootModule, modulesSet, runtime, moduleGraph) {
    ...
    const getConcatenatedImports = (module) => {
      ...
      for (const { connection } of references) {
        const runtimeCondition = filterRuntime(
          runtime,
          (r) => connection.isTargetActive(r),
        );
  ...
  codeGeneration({
          ...
          for (const variable of info.moduleScope.variables) {
            ...
            if (allUsedNames.has(name) || usedNames.has(name)) {
              ...
              const allIdentifiers = new Set(
                references.map((r) => r.identifier).concat(
              ...
              for (const identifier of allIdentifiers) {
                ...
                if (path && path.length > 1) {
                  const maybeProperty = path[1].type === "AssignmentPattern" &&
                      path[1].left === path[0]
                    ? path[2]
                    : path[1];
                  if (
    ...
    if (exportsMap.size > 0) {
      ...
      result.add(`\n// EXPORTS\n`);
      result.add(
        `${RuntimeGlobals.definePropertyGetters}(${this.exportsArgument}, {${
          definitions.join(",")
        }\n});\n`,
      );
    }
...
${defineGetters}`,
    ...
    for (const rawInfo of modulesWithInfo) {
      ...
      switch (info.type) {
        ...
        case "external": {
          ...
          result.add(
            `var ${info.name} = __webpack_require__(${
              JSON.stringify(
  ...
  updateHash(hash, context) {
    ...
    for (
      const info of this._createConcatenationList(
        this.rootModule,
        this._modules,
