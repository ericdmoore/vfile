...
const HarmonyImportDependency = require(
  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap(
      ...
      (compilation) => {
        ...
        const formatBailoutWarning = (module, problem) =>
          (requestShortener) => {
        ...
        compilation.hooks.optimizeChunkModules.tapAsync(
          ...
          (allChunks, modules, callback) => {
            ...
            for (const currentRoot of relevantModules) {
              ...
              const activeRuntime = filteredRuntime === true
                ? chunkRuntime
              ...
              if (!currentConfiguration.isEmpty()) {
                ...
                for (
                  const warning of currentConfiguration.getWarningsSorted()
                ) {
            ...
            logger.debug(
              `${concatConfigurations.length} successful concat configurations (avg size: ${
                ...
                const integrate = () => {
                  ...
                  for (const m of modules) {
                    ...
                    if (m !== rootModule) {
                      ...
                      for (
                        const chunk of chunkGraph.getModuleChunksIterable(
                          rootModule,
                        )
                      ) {
                  ...
                  moduleGraph.moveModuleConnections(
                    ...
                    newModule,
                    (c) => {
                      const otherModule = c.module === rootModule
                        ? c.originModule : c.module;
  ...
  _tryToAdd(
    ...
    if (missingChunks.length > 0) {
      const problem = (requestShortener) => {
        ...
        } is not in the same chunk(s) (expected in chunk(s) ${
          missingChunksList.join(
            ", ",
          )
    ...
    const incomingConnectionsFromNonModules = incomingConnections.get(null) ||
      incomingConnections.get(undefined);
    if (incomingConnectionsFromNonModules) {
      const activeNonModulesConnections = incomingConnectionsFromNonModules
        .filter((connection) => {
      ...
      if (activeNonModulesConnections.length > 0) {
        const problem = (requestShortener) => {
          const importingExplanations = new Set(
            activeNonModulesConnections.map((c) => c.explanation).filter(
              Boolean,
            ),
          );
    ...
    if (otherChunkModules.length > 0) {
      const problem = (requestShortener) => {
        ...
        } is referenced from different chunks by these modules: ${
          names.join(
            ", ",
          )
        }`;
    ...
    if (nonHarmonyConnections.size > 0) {
      const problem = (requestShortener) => {
        ...
        } is referenced from these modules with unsupported syntax: ${
          names.join(
            ", ",
          )
        }`;
    ...
    if (runtime !== undefined && typeof runtime !== "string") {
      ...
      if (otherRuntimeConnections.length > 0) {
        const problem = (requestShortener) => {
          ...
          } is runtime-dependent referenced by these modules: ${
            Array.from(
