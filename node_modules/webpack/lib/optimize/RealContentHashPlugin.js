        ...
        async () => {
          ...
          await Promise.all(
            assetsWithInfo.map(async (asset) => {
              ...
              [asset.referencedHashes, asset.ownHashes] = await cacheAnalyse
                .providePromise(name, etag, () => {
...
${
                referencingAssets
                  .map((a) => {
                    const match = new RegExp(`.{0,20}${quoteMeta(hash)}.{0,20}`)
                      .exec(
                        a.content,
                      );
