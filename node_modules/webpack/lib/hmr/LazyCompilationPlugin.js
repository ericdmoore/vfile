...
const CommonJsRequireDependency = require(
...
class LazyCompilationProxyModule extends Module {
  ...
  readableIdentifier(requestShortener) {
    return `lazy-compilation-proxy ${
      this.originalModule.readableIdentifier(
        requestShortener,
      )
    }`;
  }
  ...
  codeGeneration({ runtimeTemplate, chunkGraph, moduleGraph }) {
    ...
    const keepActive = Template.asString([
      `var dispose = client.keepAlive({ data: data, active: ${
        JSON.stringify(
          !!block,
        )
        ...
        normalModuleFactory.hooks.module.tap(
          ...
          (originalModule, createData, resolveData) => {
            if (
              ...
              const originModule = compilation.moduleGraph.getParentModule(
                hmrDep,
              );
            ...
            if (
              /webpack[/\\]hot[/\\]|webpack-dev-server[/\\]client|webpack-hot-middleware[/\\]client/
                .test(
                  resolveData.request,
                ) ||
