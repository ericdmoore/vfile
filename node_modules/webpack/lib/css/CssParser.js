...
const CssLocalIdentifierDependency = require(
  "../dependencies/CssLocalIdentifierDependency",
);
const CssSelfLocalIdentifierDependency = require(
    ...
    const parseExports = (input, pos) => {
      ...
      if (cc !== CC_LEFT_CURLY) {
        throw new Error(
          `Unexpected ${
            input[pos]
          } at ${pos} during parsing of ':export' (expected '{')`,
        );
      }
      pos++;
      pos = walkCssTokens.eatWhitespaceAndComments(input, pos);
      for (;;) {
        ...
        if (input.charCodeAt(pos) !== CC_COLON) {
          throw new Error(
            `Unexpected ${
              input[pos]
            } at ${pos} during parsing of export name in ':export' (expected ':')`,
          );
        }
        pos++;
        if (pos === input.length) return pos;
        ...
        if (cc === CC_SEMICOLON) {
          ...
          throw new Error(
            `Unexpected ${
              input[pos]
            } at ${pos} during parsing of export value in ':export' (expected ';' or '}')`,
          );
        }
        ...
        if (name === "@keyframes") {
          ...
          if (input.charCodeAt(pos) !== CC_LEFT_CURLY) {
            throw new Error(
              `Unexpected ${
                input[pos]
              } at ${pos} during parsing of @keyframes (expected '{')`,
            );
          }
