          ...
          compilation.warnings.push(
            new Error(
              `chunk ${chunk.name || chunk.id}\nConflicting order between ${
                hasFailed.readableIdentifier(
                  compilation.requestShortener,
                )
              } and ${
                selectedModule.readableIdentifier(
                  compilation.requestShortener,
                )
              }`,
            ),
  ...
  renderChunk({
    ...
    for (const module of modules) {
      try {
        ...
        const s = codeGenResult.sources.get("css") ||
          codeGenResult.sources.get("css-import");
        if (s) {
          source.add(s);
          source.add("\n");
        }
        const exports = codeGenResult.data &&
          codeGenResult.data.get("css-exports");
