...
class ChunkGraph {
  ...
  getChunkModuleIdMap(chunk, filterFn, includeAllChunks = false) {
    ...
    for (
      const asyncChunk of includeAllChunks
      ...
      let array;
      for (
        const module of this.getOrderedChunkModulesIterable(
          asyncChunk,
  ...
  getChunkSize(chunk, options = {}) {
    ...
    const chunkOverhead = typeof options.chunkOverhead === "number"
  ...
  integrateChunks(chunkA, chunkB) {
    ...
    if (chunkA.name && chunkB.name) {
      if (
        ...
        if (chunkA.name.length !== chunkB.name.length) {
          chunkA.name = chunkA.name.length < chunkB.name.length
            ? chunkA.name
            : chunkB.name;
        } else {
    ...
    for (
      const [module, chunkGroup] of Array.from(
  ...
  _getModuleHashInfo(module, hashes, runtime) {
    if (!hashes) {
      throw new Error(
        `Module ${module.identifier()} has no hash info for runtime ${
          runtimeToString(
            runtime,
          )
      ...
      if (hashInfoItems.size !== 1) {
        throw new Error(
          `No unique hash info entry for unspecified runtime for ${module.identifier()} (existing runtimes: ${
            Array.from(
              hashes.keys(),
              (r) => runtimeToString(r),
            ).join(", ")
...
Caller might not support runtime-dependent code generation (opt-out via optimization.usedExports: "global").`,
  ...
  getModuleRuntimeRequirements(module, runtime) {
    ...
    const runtimeRequirements = cgm.runtimeRequirements &&
      cgm.runtimeRequirements.get(runtime);
  ...
  _getModuleGraphHashWithConnections(cgm, module, runtime) {
    ...
    return cgm.graphHashesWithConnections.provide(runtime, () => {
      ...
      const connectedModulesInOrder = connectedModules.size > 1
