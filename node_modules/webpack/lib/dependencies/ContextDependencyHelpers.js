...
exports.create = (Dep, range, param, expr, options, contextOptions, parser) => {
  if (param.isTemplateString()) {
    ...
    let postfixRaw = param.quasis.length > 1
    ...
    const innerRegExp = options.wrappedContextRegExp.source +
      innerQuasis
    ...
    param.parts.forEach((part, i) => {
      if (i % 2 === 0) {
        ...
        if (i === 0) {
          ...
          value = (param.templateStringKind === "cooked"
            ? "`"
            : "String.raw`") +
            value;
    ...
    dep.critical = options.wrappedContextCritical &&
    ...
    let prefixRaw = param.prefix && param.prefix.isString()
      ? param.prefix.string
      : "";
    let postfixRaw = param.postfix && param.postfix.isString()
      ? param.postfix.string
      : "";
    const prefixRange = param.prefix && param.prefix.isString()
      ? param.prefix.range
      : null;
    const postfixRange = param.postfix && param.postfix.isString()
      ? param.postfix.range
      : null;
    const valueRange = param.range;
    const { context, prefix } = splitContextFromPrefix(prefixRaw);
    ...
    const regExp = new RegExp(
      `^${quoteMeta(prefix)}${options.wrappedContextRegExp.source}${
        quoteMeta(
          postfix,
        )
      }$`,
    ...
    dep.loc = expr.loc;
    dep.critical = options.exprContextCritical &&
