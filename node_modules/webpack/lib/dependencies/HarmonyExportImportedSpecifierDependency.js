          ...
          for (const [request, exports] of conflicts) {
            ...
            errors.push(
              new HarmonyLinkingError(
                `The requested module '${this.request}' contains conflicting star exports for the ${
                  exports.length > 1 ? "names" : "name"
                } ${
                  exports
                    .map((e) => `'${e}'`)
                    .join(", ")
                } with the previous requested module '${request}'`,
              ),
            );
          }
        }
...
HarmonyExportImportedSpecifierDependency.Template =
  class HarmonyExportImportedSpecifierDependencyTemplate extends (
      ...
      switch (mode.type) {
        ...
        case "reexport-dynamic-default":
          initFragments.push(
            this.getReexportFragment(
              ...
              moduleGraph.getExportsInfo(module).getUsedName(
                mode.name,
                runtime,
              ),
              importVar,
        ...
        case "dynamic-reexport": {
          const ignored = mode.hidden
            ? combine(mode.ignored, mode.hidden)
            : mode.ignored;
          const modern = runtimeTemplate.supportsConst() &&
          ...
          if (ignored.size > 1) {
            ...
            content += `if(__WEBPACK_IMPORT_KEY__ !== ${
              JSON.stringify(
                first(ignored),
              )
            }) `;
    ...
    getReexportFakeNamespaceObjectFragments(
      ...
      map.set(
        key,
        `/* reexport fake namespace object from non-harmony */ ${name}_namespace_cache || (${name}_namespace_cache = ${RuntimeGlobals.createFakeNamespaceObject}(${name}${
          fakeType ? `, ${fakeType}` : ""
        }))`,
      );
    ...
    getConditionalReexportStatement(
      ...
      return `if(${RuntimeGlobals.hasOwnProperty}(${name}, ${
        JSON.stringify(
          valueKey[0],
        )
      })) ${RuntimeGlobals.definePropertyGetters}(${exportsName}, { ${
        JSON.stringify(
          key,
        )
