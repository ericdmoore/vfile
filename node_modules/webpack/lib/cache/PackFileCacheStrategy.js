...
class PackContent {
  ...
  get(identifier) {
    ...
    if (lazyName) {
      ...
      timeMessage = `restore cache content ${lazyName} (${
        formatSize(
          this.getSize(),
        )
      })`;
      this.logger.log(
        `starting to restore cache content ${lazyName} (${
          formatSize(
            this.getSize(),
          )
  ...
  unpack(reason) {
    ...
    if (this.lazy) {
      ...
      if (lazyName) {
        // only log once
        this.lazyName = undefined;
        timeMessage = `unpack cache content ${lazyName} (${
          formatSize(
            this.getSize(),
          )
        })`;
        this.logger.log(
          `starting to unpack cache content ${lazyName} (${
            formatSize(
              this.getSize(),
            )
          }) because ${reason}`,
...
class PackFileCacheStrategy {
  ...
  constructor({
    ...
    this._extension = compression === "brotli"
      ? ".pack.br"
      : compression === "gzip"
      ? ".pack.gz"
      : ".pack";
                    ...
                    this.fileSystemInfo.createSnapshot(
                      ...
                      (err, snapshot) => {
                        ...
                        if (this.buildSnapshot) {
                          this.buildSnapshot = this.fileSystemInfo
                            .mergeSnapshots(
                              this.buildSnapshot,
                              snapshot,
                            );
