...
const keyedNestedConfig = (value, fn, customKeys) => {
  const result = value === undefined ? {} : Object.keys(value).reduce(
    (obj, key) => (
      (obj[key] = (
...
const getNormalizedWebpackOptions = (config) => {
  return {
    ...
    entry: config.entry === undefined
      ...
      ? (
        (fn) => () => Promise.resolve().then(fn).then(getNormalizedEntryStatic)
      )(config.entry)
    ...
    optimization: nestedConfig(config.optimization, (optimization) => {
      return {
        ...
        emitOnErrors: optimization.noEmitOnErrors !== undefined
    ...
    recordsInputPath: config.recordsInputPath !== undefined
      ? config.recordsInputPath
      : config.recordsPath,
    recordsOutputPath: config.recordsOutputPath !== undefined
