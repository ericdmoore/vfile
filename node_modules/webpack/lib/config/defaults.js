  ...
  if (futureDefaults) {
    ...
    A(snapshot, "immutablePaths", () => {
      if (process.versions.pnp === "1") {
        const match =
          /^(.+?[\\/]v4)[\\/]npm-watchpack-[^\\/]+-[\da-f]{40}[\\/]node_modules[\\/]/
            .exec(
              require.resolve("watchpack"),
            );
        if (match) {
...
const getResolveDefaults = ({ cache, context, targetProperties, mode }) => {
  ...
  const browserField = tp && tp.web &&
    (!tp.node || (tp.electron && tp.electronRenderer));
...
const applyInfrastructureLoggingDefaults = (infrastructureLogging) => {
  ...
  const tty = /** @type {any} */ (infrastructureLogging.stream).isTTY &&
