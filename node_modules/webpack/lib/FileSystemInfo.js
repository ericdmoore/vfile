...
class SnapshotOptimization {
  ...
  getStatisticMessage() {
    ...
    }% (${this._statItemsShared}/${total}) entries shared via ${this._statSharedSnapshots} shared snapshots (${
  ...
  optimize(newSnapshot, capturedFiles) {
    ...
    optimizationEntries:
    for (const optimizationEntry of optimizationEntries) {
...
class FileSystemInfo {
  ...
  resolveBuildDependencies(context, deps, callback) {
    const { resolveContext, resolveEsm, resolveCjs, resolveCjsAsChild } = this
      ._createBuildDependenciesResolvers();
    ...
    const jobToString = (job) => {
      switch (job.type) {
        case RBDT_RESOLVE_CJS:
          return `resolve commonjs ${job.path}${
            expectedToString(
              job.expected,
            )
          }`;
        ...
        case RBDT_RESOLVE_CJS_FILE:
          return `resolve commonjs file ${job.path}${
            expectedToString(
              job.expected,
            )
          }`;
        case RBDT_RESOLVE_ESM_FILE:
          return `resolve esm file ${job.path}${
            expectedToString(
              job.expected,
            )
          }`;
  ...
  checkResolveResultsValid(resolveResults, callback) {
    const { resolveCjs, resolveCjsAsChild, resolveEsm, resolveContext } = this
      ._createBuildDependenciesResolvers();
    asyncLib.eachLimit(
      resolveResults,
      20,
