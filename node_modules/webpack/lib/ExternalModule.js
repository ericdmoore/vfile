...
const getSourceForCommonJsExternal = (moduleAndSpecifiers) => {
  ...
  return {
    expression: `require(${JSON.stringify(moduleName)})${
      propertyAccess(
        moduleAndSpecifiers,
        1,
      )
...
const getSourceForCommonJsExternalInNodeModule = (moduleAndSpecifiers) => {
  ...
  if (!Array.isArray(moduleAndSpecifiers)) {
    return {
      expression: `__WEBPACK_EXTERNAL_createRequire(import.meta.url)(${
        JSON.stringify(
          moduleAndSpecifiers,
        )
      })`,
...
const getSourceForModuleExternal = (
  ...
  const baseAccess = `${initFragment.getNamespaceIdentifier()}${
    propertyAccess(
      moduleAndSpecifiers,
      1,
    )
  ...
  return {
    expression,
    init:
      `var x = y => { var x = {}; ${RuntimeGlobals.definePropertyGetters}(x, y); return x; }\nvar y = x => () => x`,
...
const checkExternalVariable = (variableName, request, runtimeTemplate) => {
  return `if(typeof ${variableName} === 'undefined') { ${
    runtimeTemplate.throwMissingModuleErrorBlock(
      { request },
    )
  } }\n`;
};
...
const getSourceForAmdOrUmdExternal = (
  ...
  const externalVariable = `__WEBPACK_EXTERNAL_MODULE_${
...
class ExternalModule extends Module {
  ...
  updateHash(hash, context) {
    ...
    hash.update(
      `${this.externalType}${JSON.stringify(this.request)}${
        this.isOptional(
