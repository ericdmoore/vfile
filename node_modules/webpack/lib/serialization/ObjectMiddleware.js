...
const NullPrototypeObjectSerializer = require(
...
class ObjectMiddleware extends SerializerMiddleware {
  ...
  serialize(data, context) {
      ...
      return arr
        .map((item) => {
          if (typeof item === "string") {
            if (item.length > 100) {
              return `String ${
                JSON.stringify(item.slice(0, 100)).slice(
                  0,
                  -1,
                )
              }..."`;
    ...
    const process = (item) => {
      if (Buffer.isBuffer(item)) {
        ...
        if (serializedData !== undefined) {
          ...
          const data = SerializerMiddleware.serializeLazy(
            item,
    ...
    try {
      ...
      data = result = referenceable = bufferDedupeMap = objectTypeLookup = ctx =
        undefined;
    }
  }
