...
class CodeGenerationResults {
  ...
  get(module, runtime) {
    ...
    if (entry === undefined) {
      throw new Error(
        `No code generation entry for ${module.identifier()} (existing entries: ${
          Array.from(
            this.map.keys(),
            (m) => m.identifier(),
          ).join(", ")
    ...
    if (runtime === undefined) {
      if (entry.size > 1) {
        ...
        if (results.size !== 1) {
          throw new Error(
            `No unique code generation entry for unspecified runtime for ${module.identifier()} (existing runtimes: ${
              Array.from(
                entry.keys(),
                (r) => runtimeToString(r),
              ).join(", ")
...
Caller might not support runtime-dependent code generation (opt-out via optimization.usedExports: "global").`,
    ...
    if (result === undefined) {
      throw new Error(
        `No code generation entry for runtime ${
          runtimeToString(
            runtime,
          )
        } for ${module.identifier()} (existing runtimes: ${
          Array.from(
            entry.keys(),
            (r) => runtimeToString(r),
          ).join(", ")
