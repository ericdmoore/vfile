...
var sanitizeRange = function sanitizeRange(range) {
  return range.replace(REGEX_REGEXP_RANGE, function (match, from, to) {
    return from.charCodeAt(0) <= to.charCodeAt(0)
      ? match // Invalid range (out of order) which is ok for gitignore rules but
  ...
  [ // > The range notation, e.g. [a-zA-Z],
    ...
    function (match, leadEscape, range, endEscape, close) {
      return leadEscape === ESCAPE // '\\[bar]' -> '\\\\[bar\\]'
        ? "\\[".concat(range).concat(cleanRangeBackSlash(endEscape)).concat(
          close,
        )
        : close === "]"
        ? endEscape.length % 2 === 0 // A normal case, and it is a range notation
          // '[bar]'
          // '[bar\\\\]'
...
var checkPath = function checkPath(path, originalPath, doThrow) {
  if (!isString(path)) {
    return doThrow(
      "path must be a string, but got `".concat(originalPath, "`"),
      TypeError,
    );
  ...
  if (checkPath.isNotRelative(path)) {
    ...
    return doThrow(
      "path should be a ".concat(r, ' string, but got "').concat(
        originalPath,
        '"',
      ),
      RangeError,
...
var Ignore = /*#__PURE__*/ function () {
  function Ignore() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined
      ...
      allowRelativePaths = _ref$allowRelativePat === void 0
        ? false
...
if (
  ...
  var makePosix = function makePosix(str) {
    return /^\\\\\?\\/.test(str) || /[\0-\x1F"<>\|]+/.test(str)
      ? str
      : str.replace(/\\/g, "/");
  };
