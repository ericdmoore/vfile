...
(function (global, factory) {
  ...
  function makeSnippet(mark, options) {
    ...
    var result = "", i, line;
    var lineNoLength =
      Math.min(mark.line + options.linesAfter, lineEnds.length).toString()
        .length;
  ...
  function mergeMappings(state, destination, source, overridableKeys) {
    ...
    for (
      index = 0, quantity = sourceKeys.length; index < quantity; index += 1
    ) {
      key = sourceKeys[index];
  ...
  function storeMappingPair(
    ...
    if (keyTag === "tag:yaml.org,2002:merge") {
      if (Array.isArray(valueNode)) {
        for (
          index = 0, quantity = valueNode.length; index < quantity; index += 1
        ) {
  ...
  function readBlockMapping(state, nodeIndent, flowIndent) {
    ...
    while (ch !== 0) {
      ...
      //
      if (
        (ch === 0x3F /* ? */ || ch === 0x3A /* : */) && is_WS_OR_EOL(following)
      ) {
        if (ch === 0x3F /* ? */) {
          if (atExplicitKey) {
            storeMappingPair(
  ...
  function State(options) {
    ...
    this.quotingType = options["quotingType"] === '"'
      ? QUOTING_TYPE_DOUBLE
      : QUOTING_TYPE_SINGLE;
    this.forceQuotes = options["forceQuotes"] || false;
    this.replacer = typeof options["replacer"] === "function"
      ? options["replacer"]
      : null;
  ...
  function testImplicitResolving(state, str) {
    ...
    for (
      index = 0, length = state.implicitTypes.length; index < length; index += 1
    ) {
  ...
  function getDuplicateReferences(object, state) {
    ...
    for (
      index = 0, length = duplicatesIndexes.length; index < length; index += 1
    ) {
    ...
    if (object !== null && typeof object === "object") {
      ...
      if (index !== -1) {
        ...
        if (Array.isArray(object)) {
          ...
          for (
            index = 0, length = objectKeyList.length; index < length; index += 1
          ) {
            inspectNode(
