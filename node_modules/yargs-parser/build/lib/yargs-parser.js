  ...
  parse(argsInput, options) {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg !== "--" && isUnknownOptionAsArg(arg)) {
        ...
        arg.match(negatedBoolean) && configuration["boolean-negation"]
      ) {
    ...
    if (configuration["strip-aliased"]) {
      [].concat(...Object.keys(aliases).map((k) => aliases[k])).forEach(
        (alias) => {
          if (configuration["camel-case-expansion"] && alias.includes("-")) {
            delete argv[
              alias.split(".").map((prop) => camelCase(prop)).join(".")
            ];
          }
          delete argv[alias];
        },
    ...
    function eatArray(i, key, args, argAfterEqualSign) {
      ...
      if (checkAllAliases(key, flags.bools) && !(/^(true|false)$/.test(next))) {
        ...
        isUndefined(next) ||
        (isUndefined(argAfterEqualSign) && /^-/.test(next) &&
          !negative.test(next) && !isUnknownOptionAsArg(next))
      ) {
        ...
        for (let ii = i + 1; ii < args.length; ii++) {
          if (
            ...
            (nargsCount && typeof nargsCount === "number" &&
    ...
    function maybeCoerceNumber(key, value) {
      ...
      if (
        ...
        !checkAllAliases(key, flags.bools) && !Array.isArray(value)
      ) {
    ...
    function setConfigObject(config, prev) {
      Object.keys(config).forEach(function (key) {
        ...
        if (
          ...
          !Array.isArray(value) && configuration["dot-notation"]
        ) {
