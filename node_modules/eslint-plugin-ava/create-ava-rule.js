...
module.exports = () => {
  ...
  const predefinedRules = {
    ImportDeclaration: (node) => {
      if (
        !isTestFile && avaImportDeclarationAsts.some((ast) =>
          isDeepStrictEqual(espurify(node), ast)
        )
      ) {
        isTestFile = true;
      }
    ...
    VariableDeclarator: (node) => {
      if (
        !isTestFile && avaVariableDeclaratorAsts.some((ast) =>
          isDeepStrictEqual(espurify(node), ast)
        )
      ) {
        isTestFile = true;
  ...
  return {
    hasTestModifier: (mod) =>
      getTestModifierNames(currentTestNode).includes(mod),
    ...
    merge: (customHandlers) =>
      enhance.mergeVisitors([predefinedRules, customHandlers]),
  };
};
