...
const create = (context) => {
  ...
  const findReference = (name) => {
    const reference = context.getScope().references.find((reference) =>
  ...
  const booleanHandler = (node) => {
    ...
    const assertion = ["true", "truthy"].includes(node.callee.property.name)
      ? "regex"
      : "notRegex";
    ...
    const fix = (fixer) => {
      ...
      return [
        ...
        fixer.replaceText(
          firstArg,
          `${source.getText(variable)}, ${source.getText(lookup)}`,
        ),
      ];
    };
    ...
    const booleanFixer = (assertion) =>
      (fixer) => {
        ...
        return [
          ...
          fixer.replaceText(
            secondArg,
            `${source.getText(regex.callee.object)}`,
          ),
        ];
      };
