...
const isStaticTemplateLiteral = (node) =>
  node.expressions.every((expression) => isStatic(expression));

const isStatic = (node) =>
  node.type === "Literal" ||
  (node.type === "TemplateLiteral" && isStaticTemplateLiteral(node)) ||
  (node.type === "BinaryExpression" && isStatic(node.left) &&
    isStatic(node.right));
...
const create = (context) => {
  ...
  return ava.merge({
    CallExpression: visitIf([
      ...
      const titleNode = args.length > 1 || ava.hasTestModifier("todo")
        ? args[0]
        : undefined;
