    ...
    if (this.hasGlob) {
      ...
      this.unmatchedGlob = !this.dirParts.some((parts) => {
        return parts.every((part, i) => {
          ...
          return globstar || !entryParts[0][i] ||
            anymatch(part, entryParts[0][i], ANYMATCH_OPTS);
        });
      });
    }
    return !this.unmatchedGlob &&
...
class FSWatcher extends EventEmitter {
  ...
  async _emit(event, path, val1, val2, val3) {
    ...
    if (
      awf && (event === EV_ADD || event === EV_CHANGE) && this._readyEmitted
    ) {
  ...
  _isIgnored(path, stats) {
    ...
    if (!this._userIgnored) {
      ...
      const list = this._getGlobIgnored().map(normalizeIgnored(cwd)).concat(
        ignored,
        paths,
      );
  ...
  _getWatchHelpers(path, depth) {
    const watchPath = depth || this.options.disableGlobbing || !isGlob(path)
      ? path
      : globParent(path);
