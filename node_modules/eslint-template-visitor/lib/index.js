...
class Template {
  constructor(ast, options, templates) {
    if (typeof ast === "string") {
      ...
      ast = firstNode.type === "ExpressionStatement"
...
class TemplateManager {
  ...
  _getSpreadVariableNode(templateNode) {
    ...
    return variable instanceof SpreadVariable ? templateNode : undefined;
  }
  ...
  _nodeMatches(templateNode, node, context) {
    ...
    if (variable) {
      ...
      if (
        previousMatches.every((previousMatchNode) =>
          nodeEquals(previousMatchNode, node)
        )
      ) {
        if (variable instanceof VariableDeclarationVariable) {
          const templateDeclarations = templateNode.declarations.slice(1);
          if (
            !this._nodeArraysMatch(
  ...
  _spreadVariableMatches(templateNode, nodes, context) {
    ...
    if (
      previousMatches.every((previousMatchNodes) =>
        everyNodeEquals(previousMatchNodes, nodes)
      )
    ) {
