...
const babelEslintWorkarounds = require(
  "../test/_babel-eslint-parser-workarounds",
);
...
templateFoundInMacro.title = (
  ...
  return `\`${templateSource}\` ${
    expectedToMatch ? "should be found in" : "should not be found in"
  } \`${source}\``;
};

const templateMatchesMacro = (
  t,
  templateSource,
  source,
  expectedToMatch = true,
...
templateMatchesMacro.title = (
  ...
  return `\`${templateSource}\` ${
    expectedToMatch ? "should match" : "should not match"
  } \`${source}\``;
};
...
test("variable values", (t) => {
  ...
  // Should not match
  recurse.visit(
    parse("bar.foo(argument)", parserOptions),
    templates.visitor(visitor),
  );
  recurse.visit(
    parse("bar.foo(...arguments)", parserOptions),
    templates.visitor(visitor),
  );
});
...
test("`spreadVariable` matching arguments", (t) => {
  ...
  recurse.visit(
    parse("receiver.method()", parserOptions),
    templates.visitor(visitor),
  );
  ...
  recurse.visit(
    parse("receiver.method(onlyArgument)", parserOptions),
    templates.visitor(visitor),
  );
  ...
  recurse.visit(
    parse("receiver.method(argument1, argument2)", parserOptions),
    templates.visitor(visitor),
  );
  ...
  recurse.visit(
    parse("receiver.method(...arguments)", parserOptions),
    templates.visitor(visitor),
  );
...
test("`spreadVariable` matching statements", (t) => {
  ...
  recurse.visit(
    parse("() => { onlyStatement; }", parserOptions),
    templates.visitor(visitor),
  );
  ...
  recurse.visit(
    parse("() => { statement1; statement2 }", parserOptions),
    templates.visitor(visitor),
  );
...
test("`variableDeclarationVariable` matching variable declarations", (t) => {
  ...
  const visitor = {
    [template](node) {
      const variableDeclarationNode = template.context.getMatch(
...
test("`variableDeclarationVariable` unification", (t) => {
  ...
  recurse.visit(
    parse("() => { var x; var x; }", parserOptions),
    templates.visitor(visitor),
  );
...
if (PARSER === "@babel/eslint-parser") {
  test("dynamic import", (t) => {
    ...
    const source =
      "async () => { await import('bar'); await import('foo'); await import('qux'); }";
...
test("fuzzing", (t) => {
  ...
  times(() => {
    ...
    const randomEspreeSafeShiftAST = shiftToEspreeSafe(
      dropExtraTopLevelNodes(randomShiftAST),
    );
    const randomJS =
      shiftCodegen(randomEspreeSafeShiftAST, new FormattedCodeGen()) ||
      '"empty program";';

    try {
      const { shouldSkip } = babelEslintWorkarounds(
        babelEslintParse(randomJS, parserOptions),
      );

      if (shouldSkip) {
        console.warn(
          "Ignored a random script due to a `@babel/eslint-parser` bug (this is fine):",
          randomJS,
        );
        skippedTests += 1;
        return;
    ...
    if (!called) {
      ...
      throw new Error(
        "Fuzzing test failed. This error is thrown just to stop this long test early.",
      );
    }
  }, totalTests);
