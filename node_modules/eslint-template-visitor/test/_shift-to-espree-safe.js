...
class NoUndeclaredExportReducer extends ScopeLookupCloneReducer {
  reduceExportLocals(node, { namedExports, ...rest }) {
    return {
      ...node,
      ...rest,
      namedExports: namedExports.filter((export_) =>
        !export_._isUndeclaredVariable
      ),
    };
  }
  ...
  reduceExportLocalSpecifier(
    node,
    { name: { _isUndeclaredVariable, ...name }, ...rest },
  ) {
    return {
      ...node,
      ...rest,
...
class NoNonStrictFeaturesReducer extends removingHOR(CloneReducer) {
  ...
  reduceUnaryExpression(node) {
    if (
      node.operator === "delete" && node.operand.type === "IdentifierExpression"
    ) {
      return node.operand;
    }
