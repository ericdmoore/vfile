...
function loadLegacyConfigFile(filePath) {
  ...
  try {
    return yaml.load(
      stripComments(readFile(filePath)),
...
function createContext(
  ...
  const name = providedName ||
    ...
    "";
  const pluginBasePath = resolvePluginsRelativeTo ||
    (filePath && path.dirname(filePath)) ||
    cwd;
...
class ConfigArrayFactory {
  ...
  *_normalizeObjectConfigDataBody(
  ...
  _loadExtendedPluginConfig(extendName, ctx) {
    ...
    const configData = plugin.definition &&
  ...
  _loadPlugin(name, ctx) {
    ...
    const plugin = additionalPluginPool.get(request) ||
  ...
  *_takeFileExtensionProcessors(plugins, ctx) {
    for (const pluginId of Object.keys(plugins)) {
      const processors = plugins[pluginId] &&
