...
/* c8 ignore next 4 */
const chars = process.platform === "win32"
  ? { error: "×", warning: "‼" }
  : { error: "✖", warning: "⚠" };
...
function format(map, one, options) {
  /** @type {boolean} */
  const enabled = options.color === undefined || options.color === null
    ? color
    : options.color;
  /** @type {Array<string>} */
  const lines = [];
  ...
  while (++index < map.rows.length) {
    ...
    if ("type" in row) {
      ...
      line = one && !options.defaultName && !row.file.history[0]
        ? ""
        : (enabled
          ? "\u001B[4m" /* Underline. */ +
            (stats.fatal ? "\u001B[31m" /* Red. */ : stats.total
              ? "\u001B[33m" /* Yellow. */
              : "\u001B[32m") /* Green. */ +
            line +
            "\u001B[39m\u001B[24m"
      ...
      if (!stats.total) {
        line = (line ? line + ": " : "") +
          (row.file.stored
            ? enabled ? "\u001B[33mwritten\u001B[39m" /* Yellow. */ : "written"
            : "no issues found");
      }
