...
const normalizeExpectedType = (type, defaultValue) => {
  ...
  if (
    arrayTypes.includes(inferredType) && Array.isArray(defaultValue) &&
    ...
    if (isPlainObject(value)) {
      ...
      if (type) {
        if (!availableTypes.includes(type)) {
          throw new TypeError(
            `Expected type of "${key}" to be one of ${
              prettyPrint(availableTypes)
      ...
      if ({}.hasOwnProperty.call(props, "default")) {
        ...
        if (expectedType && expectedType !== defaultType) {
          throw new TypeError(
            `Expected "${key}" default value to be of type "${expectedType}", got ${
              prettyPrint(defaultType)
            }`,
          );
        }
