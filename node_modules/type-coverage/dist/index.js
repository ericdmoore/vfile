...
async function executeCommandLine() {
  ...
  const { correctCount, totalCount, anys } =
    await (0, type_coverage_core_1.lint)(project, {
      debug,
      strict,
      enableCache,
      ignoreCatch,
  ...
  if (detail || (!noDetailWhenFailed && (atLeastFailed || isFailed))) {
    for (const { file, line, character, text } of anys) {
      const filePath = showRelativePath
        ? file
  ...
  if (atLeastFailed) {
    throw new Error(
      `The type coverage rate(${percentString}%) is lower than the target(${atLeast}%).`,
    );
  }
  if (isFailed) {
    throw new Error(
      `The type coverage rate(${percentString}%) is not the target(${is}%).`,
    );
  }
}
async function getTarget(argv) {
  ...
  if (await existsAsync(packageJsonPath)) {
    const currentPackageJson = JSON.parse(
      (await readFileAsync(packageJsonPath)).toString(),
    );
  ...
  const ignoreAsAssertion = getArgOrCfgVal([
    "ignore-as-assertion",
    "ignoreAsAssertion",
  ]);
  const ignoreTypeAssertion = getArgOrCfgVal([
    "ignore-type-assertion",
    "ignoreTypeAssertion",
  ]);
  const ignoreNonNullAssertion = getArgOrCfgVal([
    "ignore-non-null-assertion",
    "ignoreNonNullAssertion",
  ]);
  const ignoreObject = getArgOrCfgVal(["ignore-object", "ignoreObject"]);
  const ignoreEmptyType = getArgOrCfgVal([
    "ignore-empty-type",
    "ignoreEmptyType",
  ]);
  const showRelativePath = getArgOrCfgVal([
    "show-relative-path",
    "showRelativePath",
  ]);
  const historyFile = getArgOrCfgVal(["history-file", "historyFile"]);
  const noDetailWhenFailed = getArgOrCfgVal([
  ...
  if (historyFile) {
    ...
    if (await existsAsync(historyFilePath)) {
      ...
      const historyFile = JSON.parse(
        (await readFileAsync(historyFilePath)).toString(),
      );
